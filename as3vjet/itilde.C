...
double itilde::besselk(double x, int n)
{
  ...
  switch (n)
    {
    case 1: return itilde_(one,x);   break;
    case 2: return itilde_(two,x);   break;
    case 3: return itilde_(three,x); break;
    case 4: return itilde_(four,x);  break;
    case 5: return itilde_(five,x);  break;
    case 6: return itilde_(six,x);  break;
    }
  return 0;
}
...
double itilde::ibar(double x, int n)
{
  ...
  switch (n)
    ...
    case 4: return -4./x2*(pow(lx2,3)- 4.*zeta3);                      break;
    case 5: return -5./x2*(pow(lx2,4)-16.*zeta3*lx2);                  break;
    case 6: return -6./x2*(pow(lx2,5)-40.*zeta3*pow(lx2,2)-48.*zeta5); break;
    }
  return 0;
}
...
double itilde::calc(double qt, double q, int n)
{
  ...
  //cout << "itilde " << res*iq2 << " besselk " << besselk(qt/q,n) << endl;
  //cout << "itilde " << res*iq2 << " ibar " << ibar(qt/q,n) << endl;
...
double itilde::integrand(double b)
{
  ...
  if (!opts.modlog)
    ...
    val *= pow(1./btilde::p*log(pow(iq2*b2/ib02,btilde::p)+1.),in);
  */

  val *= pow(real(btilde::blog(b,sqrt(iq2))),in);  
...
double itilde::integrand_int(double b)
{
  ...
  //cout << "itilde " <<  btilde::log(b,sqrt(iq2)) << endl;
  ...
  //else
  //  jac = sqrt(pow(iq2*b2/ib02,btilde::p)/(1.+pow(iq2*b2/ib02,btilde::p)));
  ...
  if (!opts.modlog)
    ...
    jac = pow(sqrt(iq2)/iqt,btilde::p)/(1.+pow(sqrt(iq2)/iqt,btilde::p));
  val *= jac;
  */
