...
double qtint::LL5_mesqij[12];
double qtint::LL6_mesqij[12];

//!!! the option: opts.dsdqt2 is not implemented consistently for all cases !!!
...
void qtint::calc(double m, double qtmin, double qtmax, int mode)
{
  ...
  if (mode == 0 || mode == 1)
    {
      ...
      */
      ...
      double LL1,LL2,LL3,LL4,LL5,LL6;
      ...
      double Q = scales::res;
      LL1 = itilde::calc(qt,Q,1)/pow(Q,2)/q2*swtch;
      LL2 = itilde::calc(qt,Q,2)/pow(Q,2)/q2*swtch;
      LL3 = itilde::calc(qt,Q,3)/pow(Q,2)/q2*swtch;
      LL4 = itilde::calc(qt,Q,4)/pow(Q,2)/q2*swtch;
      LL5 = itilde::calc(qt,Q,5)/pow(Q,2)/q2*swtch;
      LL6 = itilde::calc(qt,Q,6)/pow(Q,2)/q2*swtch;
      ...
      for (int sp = 0; sp < mesq::totpch; sp++)
	{
	  ...
	  LL5_mesqij[sp] = LL5*real(mesq::mesqij[sp]);
	  LL6_mesqij[sp] = LL6*real(mesq::mesqij[sp]);
	}
      return;
    }
  ...
  if (opts.fixedorder && !fpc)
    {
      double LL1int, LL2int, LL3int, LL4int, LL5int, LL6int;
      ...
      LL5int = -2.*qtmin*itilde::integral(qtmin,Q,5)/pow(Q,2)/q2;
      LL6int = -2.*qtmin*itilde::integral(qtmin,Q,6)/pow(Q,2)/q2;
      ...
      for (int sp = 0; sp < mesq::totpch; sp++)
	{
	  ...
	  LL5_mesqij[sp] += LL5int*real(mesq::mesqij[sp]);
	  LL6_mesqij[sp] += LL6int*real(mesq::mesqij[sp]);
	}
      return;
    }
  ...
  */
  ...
  //In the case of resummation without lepton cuts perform the qt integration analytically (except for modlog with profile scales which are qt dependent)
  if (!opts.makecuts && opts.p > 1)
  //if (!opts.makecuts && qtmax < m*opts.dampk && qtmax < m*5 && (opts.modlog != 4))
    {
      double LL1int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,1) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,1))/pow(q2,2)*pow(a_param_.a_param_,2);
      double LL2int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,2) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,2))/pow(q2,2)*pow(a_param_.a_param_,2);
      double LL3int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,3) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,3))/pow(q2,2)*pow(a_param_.a_param_,2);
      double LL4int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,4) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,4))/pow(q2,2)*pow(a_param_.a_param_,2);
      double LL5int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,5) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,5))/pow(q2,2)*pow(a_param_.a_param_,2);
      double LL6int = 2.*(qtmax*itilde::integral(qtmax,m/a_param_.a_param_,6) - qtmin*itilde::integral(qtmin,m/a_param_.a_param_,6))/pow(q2,2)*pow(a_param_.a_param_,2);
  ...
  //In the general case (resummation, either with cuts or at high pT, or for fiducial power corrections) use a gaussian quadrature integration
  ...
  */
  ...
  if (fpc)
    {
      //qtmin = 1e-4; //hard-coded qtmin for FPC
      qtmin = m*opts.qtfpc;
      double qtmin2 = pow(qtmin,2);
      double qtmax2 = pow(qtmax,2);
      tiny = qtmin2;
      qta = 1./(1.+log(qtmax2/tiny));
      qtb = 1./(1.+log(qtmin2/tiny));
    }
  else
    {
      double qtmin2 = pow(qtmin,2);
      double qtmax2 = pow(qtmax,2);
      tiny = 1e-5; //be carefull, if qtmin^2 < tiny the phase space generation is screwed up
      //tiny = qtmin2 --> test this to avoid the problem
      qta = 1./(1.+log(qtmax2/tiny));
      qtb = 1./(1.+log(qtmin2/tiny));
    }
  double min = 0;
  ...
  //For fiducial power corrections, evaluate once cthmom without recoil
  double cth0qt0,cth1qt0,cth2qt0;
  if (fpc)
    {
      phasespace::set_qt(0.);
      omegaintegr::genV4p();
      omegaintegr::cthmoments(cth0qt0, cth1qt0, cth2qt0);
    }
  ...
  for (int i = 0; i < opts.qtintervals; i++)
      ...
      for (int j = 0; j < opts.qtrule; j++)
	{
	  ...
	  double qt2, qt, jac;
	  qt2 = tiny*exp(1./qtx - 1.);
	  qt = sqrt(qt2);
	  jac = (qtb-qta)*qt2/pow(qtx,2);
	  ...
	  //The Jacobians for qt -> qt2, y -> xF are calculated with average values when qt is integrated
	  //if (opts.dsdqt2)
	  //  jac=jac/qt/2.;
	  //
	  //if (opts.edsdp3)
	  //  {
	  //    double E = sqrt(pow(phasespace::m,2)+pow(qt,2))*cosh(phasespace::y);
	  //    jac = jac*(2.*E/opts.sroot);
	  //  }
	  ...
	  double xx = sqrt(qt2/(q2/pow(a_param_.a_param_,2))); //qt/Q;
	  double LL1 = itilde::besselk(xx,1)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;
	  double LL2 = itilde::besselk(xx,2)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;
	  double LL3 = itilde::besselk(xx,3)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;
	  double LL4 = itilde::besselk(xx,4)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;
	  double LL5 = itilde::besselk(xx,5)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;
	  double LL6 = itilde::besselk(xx,6)/pow(q2,2)*pow(a_param_.a_param_,2)*w*swtch;

	  //Use this code to allow generic modlog options
	  /*
	  phasespace::set_qt(qt); //needed for qt-dependent modified logs
	  double LL1 = itilde::calc(qt,Q,1)/pow(Q,2)/q2*w*swtch;
	  double LL2 = itilde::calc(qt,Q,2)/pow(Q,2)/q2*w*swtch;
	  double LL3 = itilde::calc(qt,Q,3)/pow(Q,2)/q2*w*swtch;
	  double LL4 = itilde::calc(qt,Q,4)/pow(Q,2)/q2*w*swtch;
	  double LL5 = itilde::calc(qt,Q,5)/pow(Q,2)/q2*w*swtch;
	  double LL6 = itilde::calc(qt,Q,6)/pow(Q,2)/q2*w*swtch;
	  */
	  ...
	  if (opts.makecuts)
	    {
	      ...
	      //For fiducial power corrections, need to evaluate the difference between with recoil and without recoil
	      if (fpc)
		{
		  cthmom0 = cth0qt0 - cthmom0;
		  cthmom1 = cth1qt0 - cthmom1;
		  cthmom2 = cth2qt0 - cthmom2;
		}

	      mesq::setmesq(cthmom0, cthmom1, cthmom2);
	      //cout << endl;
	      //cout << "qt,m,y " << phasespace::qt << "  " << phasespace::m << "  " << phasespace::y << endl;
	      //cout << "cthmoms " << cthmom0 << "  " << cthmom1 << "  " << cthmom2 << endl;
	      //cout << "LL " << LL1 << "  " << LL2 << endl;
	    }
	  //loop on born subprocesses, i.e. born incoming partons ij
	  for (int sp = 0; sp < mesq::totpch; sp++)
	    {
