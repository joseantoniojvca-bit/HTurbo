...
C  in is the label of the momentum contracted with n
      ...
      double precision n(4),p(mcfm_mxpart,4),dot,hdecay,s34,fac,
...
c   contracted with the vector n(mu)
      ...
      include 'ewcouple.f'
      integer j1,j2,j5
      double precision Asq,p(mcfm_mxpart,4),n(4),nDn,nDp1,nDp2,nDp5,
     . dot,s,t,u
      double precision aass
...
c--- so of order(1) for the Tevatron
      if (abs(nDp5).gt.1d-2*abs(p(1,4))) then 
         write(*,*) 'Error for :',j1,j2,j5
         write(*,*) 'cutoff',1d-2*abs(p(1,4))
         write(6,*) 'nDp5',nDp5
C         call flush(6)
C         stop
      endif

      Asq=(aass/(3d0*pi))**2/vevsq
