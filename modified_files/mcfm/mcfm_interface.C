...
void mcfm::init()
{
  ...
  //Dim. Reg. parameter epsilon (epinv was used in MCFM to check cancellation of epsilon poles)
  mcfm_epinv_.epinv_  = 1000;//Could consider to put 0 to improve numerical stability
  mcfm_epinv2_.epinv2_= 1000;//Could consider to put 0 to improve numerical stability
  
  string part = "tota";
  strncpy(part_.part_ , part.c_str(), part.size());

  colc_.colourchoice_=0;
  cutoff_.cutoff_=opts.mcutoff; //add to settings (this is a cut off in mcfm on invariant mass pairs between emitted and radiator)
  ...
  //(add to settings)
  alfacut_.aii_=opts.aii;//0.01;
  alfacut_.aif_=opts.aif;//0.01;
  alfacut_.afi_=opts.afi;//0.01;
  alfacut_.aff_=opts.aff;//0.01;
  ...
  //W-=>l-nu
    ...
    else if(nproc_.nproc_==4) // CJU
    {
      ...
      plabel[6]="pp";
      nwz_.nwz_=0;
      breit_.mass3_=opts.hmass;
      breit_.width3_=opts.hwidth;
      opts.rmass = opts.hmass;
      opts.rwidth = opts.hwidth;
      // cout << "mass3 = " << breit_.mass3_ << endl;
      // cout << "width3 = " << breit_.width3_ << endl;
      rvalues_.rmass_ = opts.rmass;
      rvalues_.rwidth_ = opts.rwidth;
