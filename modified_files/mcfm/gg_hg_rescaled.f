      subroutine gg_hg_rescaled(p,msq)
      implicit none
...
c     g(-p1)+g(-p2)-->H(-->  b(p3)+b~(p4))+g(p5)
      ...
      include 'ewcouple.f'
      integer j,k,iglue
      double precision msq(-nf:nf,-nf:nf),p(mcfm_mxpart,4)
      double precision ss,tt,uu,mhsq,hdecay,s(mcfm_mxpart,mcfm_mxpart)
      double precision s34,Asq,fac,mtop,mbot
