...
C
      ...
      double precision s(mcfm_mxpart,mcfm_mxpart),wgt,msq(-nf:nf,-nf:nf)
      double precision msqc(-nf:nf,-nf:nf),xmsq(0:mcfm_maxd)
      double precision flux,BrnRat,xreal,xreal2
      double precision xx1,xx2,q(mcfm_mxpart,4)
      double precision m3,m4,m5,switch,qt,m
CC
      logical cuts
      double precision x1,x2,dot,ptrans(mcfm_mxpart,4)
...
C
      ...
      logical incldip(0:mcfm_maxd),includedipole,includereal
...
CC   In this way normalization is fixed to dsigma/dqt2
      ...
      LL1=Itilde(1,xmio)/q2**2*a_param**2
      LL2=Itilde(2,xmio)/q2**2*a_param**2
      LL3=Itilde(3,xmio)/q2**2*a_param**2
      LL4=Itilde(4,xmio)/q2**2*a_param**2
...
!      if(xx20.lt.1d-5)write(*,*) "q2,xx1",q2,xx20!
      ...
      call pdf_fdist(1,xx10,facscale,fx10)
      call pdf_fdist(2,xx20,facscale,fx20)

      call pdf_fdist(1,xx10**(1-beta),facscale,fx1p)
      call pdf_fdist(2,xx20**(1-alfa),facscale,fx2p)
...
CC Non delta term in Cgg expression (with asopi normalization)
c     cju
...
CC Cqg expression (with asopi normalization)
c     cju
...
CC Regular part of Pgg*Pgg (checked !)
c     cju
...
CC Regular part of Pgg*Pqg (checked !)
c     cju
...
CC Regular part of Pqg*Pgg (checked !)
c     cju
...
CC Regular part of Pqg*Pqg (checked !)
c     cju
...
CC Cgq*Pqq (without delta term in Cqq) (checked !)
c     cju
      function CgqPqq(x)
      implicit none
      real *8 CgqPqq,x
...
CC Cgq*Pqg (without delta term in Cqq) (checked !)
c     cju
