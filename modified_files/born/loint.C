...
void loint::lint(double costh, double m, double y, int mode, double f[2])
{
  ...
  */
  ...
  if (opts.order >= 1)
    ...
    if (opts.order >= 1)
      ...
      if (opts.order >= 2)
        ...
        for (int sp = 0; sp < mesq::totpch; sp++)
        {
          ...
          if(opts.calc_type=="Sig" ) th1st += 2*resconst::C1ggdelta*tdelta;
          else if(opts.calc_type=="Int" ) th1st += H1intHSchm(costh,m2)*tdelta;
          else cout <<" SigInt not available for resummed at NLO or wrong calc_type" << endl;
    ...
    {
      xmsq = xmsq/shad/M_PI/8. ;
      xmsq_m = xmsq_m/shad/M_PI/8. ;
    }
    if (isnan_ofast(xmsq))
