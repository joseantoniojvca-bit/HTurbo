	...
	term_width = wr + // result
	  ...
	  time_width;
        ...
        void IntegrationSettings(){
            ...
            for (size_t ibound = 0; ibound < N_boundaries; ++ibound){
                if (ActiveBoundaries[ibound].size() > 2 || ActiveBoundaries[ibound].size() == 0) continue;
        ...
        String engineering_exponent(int dec_max,bool unit=false){
            ...
            if (unit && decimal_print >= 0 && decimal_print < 12 ){
                ...
                else if (decimal_print < 12) if (useUnicode) tmp << "\u03BC" << "b"; else tmp << "ub";
            } else{
                if (decimal_print!=0){
                    if (useUnicode){
                        //tmp << " \u00d7"; // mult
	  ...
	  if (value == 0 && error == 0)
        ...
        void ProcessSettings(){
            ...
            if (opts.fixedorder) {
                if (opts.order == 0)
                    ...
                    cout << "   Computing N3LO fixed order cross section" << endl;
            } else {
                if (opts.order == 0)
                    ...
                    cout << "   Computing N3LO+N3LL resummed cross section" << endl;
                else if (opts.order == 4)
                    cout << "   Computing N3LO+N4LL resummed cross section" << endl;
            }
            // Process
            if (nproc_.nproc_ == 1)
            ...
            else if(nproc_.nproc_== 3 && opts.useZ && !opts.useGamma)
                cout << "   Z ->  e-(p3)+e+(p4) production " << endl;
            else if(nproc_.nproc_== 3 && !opts.useZ && opts.useGamma)
                cout << "   gamma* ->  e-(p3)+e+(p4) production " << endl;
            else if(nproc_.nproc_== 3 && opts.useZ && opts.useGamma)
        ...
        void Higgs(){
            SettingsHeader("Higgs parameters");
            cout << Col4( "" , "1/alpha ="     , 1/opts.aemmz , ""       , 12);
            cout << Col4( "" , "vev ="     , sqrt(1./coupling::Gf/sqrt(2.0)) , ""       , 12);
            cout << Col4( "" , "Higgs mass ="      , coupling::hmass  , "GeV"    , 12);
            cout << Col4( "" , "Higgs width ="     , coupling::hwidth , "GeV"    , 12);
            cout << Col4( "" , "remove branching ratio = "     ,(opts.removebr ? "true" : "false")  , ""       , 12);
            cout << Col4( "" , "type of calculation : " , opts.calc_type, ""       , 12);
            cout << Col4( "" , "type of production = "     , opts.decay_type , ""       , 12);
            cout << Col4( "" , "top mass ="      , opts.mtop  , "GeV"    , 12);
            cout << Col4( "" , "bottom mass ="      ,opts.mbottom   , "GeV"    , 12);
            cout << Col4( "" , "charm mass ="      , opts.mcharm  , "GeV"    , 12);
            cout << Col4( "" , "tau mass ="      ,opts.mtau  , "GeV"    , 12);
            cout << Col4( "" , "Smearing : " , (opts.withsmearing ? "true" : "false"), ""       , 12);
	    ...
	    //            cout << Col4( "" , "PDF errors:" , (opts.PDFerrors ? "true" : "false") , "" );
            if (opts.doBORN && !opts.fixedorder)
            {
                ...
                else
                {
                    ...
                    cout << Col4( ""                        , "nodes for the quadrature:"     , opts.pdfrule                      , "" );
                    cout << Col4( ""                        , "Intervals for the quadrature:" , 1     , "" );
                }
            }
        }
        ...
        void AppliedCuts_Photons(){
            SettingsHeader("Cuts");
            cout << Col4( "", "apply cuts:" ,   (opts.makecuts ? "true" : "false") , "" );
            if (opts.makecuts){
                cout << Col4("" , "pt_ph_hard > "         , opts.phpt_cut_h  , "" );
                cout << Col4("" , "pt_ph_soft > "         , opts.phpt_cut_s  , "" );
                cout << Col4("" , "|eta_photon| < "      , opts.phycut   , "" );
                cout << Col4( "", "apply crack:" ,   (opts.apply_crack ? "true" : "false") , "" );
                cout << Col4("" , "|eta_crack_min| =  "    , opts.crack_eta_min  , "" );
                cout << Col4("" , "|eta_crack_max| =  "    , opts.crack_eta_max  , "" );
                cout << Col4("" , "user cuts: "     , "true"       , "" );
            }
        }
		...
		if (!opts.damp)
		...
		else
		  {
		    ...
		    cout << Col4( "" , "damp above:"         , to_string(opts.dampk)    +String("*m"), "" );
		    cout << Col4( "" , "damping width:"      , to_string(opts.dampdelta)+String("*m"), "" );
        ...
        void Settings(){
            ProcessSettings();
            CKMmatrix();
            if(opts.nproc != 4) EWparameters();
            if(opts.nproc == 4) Higgs();
            QCDsettings();
            PDFsettings();
            QTrecoil();
            if(opts.nproc == 4) AppliedCuts_Photons();
            if(opts.nproc != 4) AppliedCuts();
            ResummationDamp();
            DebugSettings();
