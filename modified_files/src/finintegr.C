                        ...
                        double &weight, const int &iter)
{
  ...
  tell_to_grid_we_are_alive(100000);
  return 0;
}
                        ...
                        double &weight, const int &iter)
{
  ...
  cout << "FININTEGR virtintegrand" << endl; // CJU FO
...
double vjlointegrand0d()
{
  double res = 0.;
  if (opts.cubacores == 0)
    {
      for (unsigned i = 0; i < sr::n4; i++)
	{
	  double x[4];
	  x[0] = {0.5+0.5*sr::x4[0+i*4]};
	  x[1] = {0.5+0.5*sr::x4[1+i*4]};
	  x[2] = {0.5+0.5*sr::x4[2+i*4]};
	  x[3] = {0.5+0.5*sr::x4[3+i*4]};
	  double f[1];
	  vjlointegrand(1, x, 1, f);
	  res += 0.5*0.5*0.5*0.5 * sr::w4[i] * f[0];
	}
      return res;
    }
  ...
  if (opts.threading == 0)
    {
#pragma omp parallel for reduction(+:res) num_threads(opts.cubacores) copyin(scale_,facscale_,qcdcouple_,vjloint::mur,vjloint::muf)
      for (unsigned i = 0; i < sr::n4; i++)
	{
	  double x[4];
	  x[0] = {0.5+0.5*sr::x4[0+i*4]};
...
{
  cout << "FININTEGR vjlointegrand" << endl; // CJU FO
...
integrand_t vjnnlovvintegrand(const int &ndim, const double x[], const int &ncomp, double f[],
                        void* userdata, const int &nvec, const int &core,
                        double &weight, const int &iter)
{
  clock_t begin_time, end_time;
  ...
  mcfmusescet_.abovecut_ = false;
  mcfmusescet_.usescet_ = true;
  origkpart_.origkpart_ = 8;//knnlo;
  kpart_.kpart_ = 8; //knnlo
  coeffonly_.coeffonly_ = true;
  double qtmn = max(max(opts.qtcut,opts.xqtcut*phasespace::mmin), phasespace::qtmin);
  jetcuts_.ptjetmin_ = qtmn;
  
  f[0] = scetint_(rre,weight,f);
...
integrand_t vjnnlovrintegrand(const int &ndim, const double x[], const int &ncomp, double f[],
                        void* userdata, const int &nvec, const int &core,
                        double &weight, const int &iter)
{
  clock_t begin_time, end_time;
  ...
  //settings for MCFM piece nnlobelow
  if (opts.nproc == 1)
    {
      as3vj_nproc_.nproc_ = 22;
      kcase_.kcase_ = 8; //kW_2jet
    }
  else if (opts.nproc == 2)
    {
      as3vj_nproc_.nproc_ = 27;
      kcase_.kcase_ = 8; //kW_2jet
    }
  else if (opts.nproc == 3)
    {
      as3vj_nproc_.nproc_ = 44;
      kcase_.kcase_ = 14; //kZ_2jet
    }

  mcfmusescet_.abovecut_ = true;
  mcfmusescet_.usescet_ = true;
  origkpart_.origkpart_ = 8;//knnlo;
  kpart_.kpart_ = 2; //kvirt
  coeffonly_.coeffonly_ = true;

  nqcdjets_.nqcdjets_=2;
  //plabel(5)='pp'
  //plabel(6)='pp'
  //plabel(7)='pp'
  
  //bveg1int_.ndim_ = ndim;  

  jetcuts_.ptjetmin_ = opts.ptjetmin;
  
  f[0] = as3vj_virtint_(rre,weight,f);
...
integrand_t vjnnlorrintegrand(const int &ndim, const double x[], const int &ncomp, double f[],
                        void* userdata, const int &nvec, const int &core,
                        double &weight, const int &iter)
{
  clock_t begin_time, end_time;
  ...
  f[0] = as3vj_realint_(rre,weight,f);
  if (isnan_ofast(f[0]))
    {
      cout << "Warning in as3vj_realint, result is " << f[0] << " consider raising mcutoffas3 " << endl;
//      cout << "phasespace is:" << endl;
//      cout << setprecision(16) << endl;
//      cout << "rre[  0 ]= " << rre[ 0] << endl;
//      cout << "rre[  1 ]= " << rre[ 1] << endl;
//      cout << "rre[  2 ]= " << rre[ 2] << endl;
//      cout << "rre[  3 ]= " << rre[ 3] << endl;
//      cout << "rre[  4 ]= " << rre[ 4] << endl;
//      cout << "rre[  5 ]= " << rre[ 5] << endl;
//      cout << "rre[  6 ]= " << rre[ 6] << endl;
//      cout << "rre[  7 ]= " << rre[ 7] << endl;
//      cout << "rre[  8 ]= " << rre[ 8] << endl;
//      cout << "rre[  9 ]= " << rre[ 9] << endl;
//      cout << "rre[ 10 ]= " << rre[10] << endl;
//      cout << "rre[ 11 ]= " << rre[11] << endl;
//      cout << "rre[ 12 ]= " << rre[12] << endl;
      f[0] = 0.;
    }
  end_time = clock();
  if (opts.timeprofile)
