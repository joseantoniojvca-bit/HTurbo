...
void scales::form(double &scale2, int ff, double m, double pt, double mjj)
{
  ...
  case 4: //see slide 27 of https://gsalam.web.cern.ch/gsalam/talks/repo/2016-03-SB+SLAC-Munich-precision.pdf
    ...
    break;
  case 5: //approximate fmu=4 at high pT, and fmu=2 at low pT
    scale2 = pow(m,2) + 2.*pow(pt,2);
    break;
  }
}
...
void scales::mcfm()
{
  ...
  mcfmscale_.scale_ = ren;
  mcfmscale_.musq_ = pow(ren,2);
  as3vj_facscale_.facscale_ = fac;
