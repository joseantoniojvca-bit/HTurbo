...
//      f[i] = 0.;
  ...
  for (int i = 0; i < ndim; i++)
    r[i]=x[i];
  ctintmc::calc(r, f, opts.order_ct_temp);
...
//      f[i] = 0.;
  ...
  if (opts.ctcpp)
	...
	ctint::calc(costh,m,qt,y,mode,f,opts.order_ct_temp);
    }
  else
    f[0]=ctint_(costh,m,qt,y,mode,f); //obsolete, remove!!!
...
//      f[i] = f[i]*jac;
  ...
  if (opts.dsdqt2 || opts.edsdp3)
    {
      cout << "Error, using CT 2d integration with dsdqt2 or edsdp3 options is a non valid approximation" << endl;
      exit(-1);
    }
...
//      f[i] = 0.;
  ...
  if (opts.dsdqt2 || opts.edsdp3 || opts.dsdxf)
    {
      cout << "Error, cannot use CT 1d integration with dsdqt2, edsdp3, or dsdxf options" << endl;
      exit(-1);
    }
  ...
  double qt = (qtmn+qtmx)/2.;
  clock_t cbt = clock();
  ctmellin::calc(m,qt,phasespace::y,mode,f);
  clock_t cet = clock();
