      subroutine plotnew(num,titlex,titley,typey)
c      implicit none 
      character* (*) titlex,titley,typey
c      integer num,num4,num43,n3,n2
      n3=3
      n2=2
      num4=1+(num-1)*4
      num43=4+(num-1)*4
      call multitop(num4,num43,n3,n2,titlex,titley,typey)
      return
      end

      subroutine plotsame(num,titlex,titley,typey)
      implicit none 
      character * (*) titlex,titley,typey
      integer num,num4,num43,n3,n2
      n3=3
      n2=2
      num4=1+(num-1)*4
      num43=4+(num-1)*4
      call mtfill(num4,num43,n3,n2,titlex,titley,typey)
      return
      end

      subroutine topfill(n,x1,y1)
      implicit none
      ...
      subroutine bookplot(num,title,nbin,b1,b2,titx,tity,typey,nt)
      implicit none 
      common/plotis/choiceplot,titlex,titley,scaley
      character * (*) title,titx,tity,typey
      character *15  titlex(50)
      character *15  titley(50)
      character *3  scaley(50)
      integer choiceplot(50)
      integer nbin,num,num4,nt
      real*8 b1,b2
      real bmin,bmax,binsize
c choiceplot=    1(newplot)  2(sameplot) 0(noplot)
      if (nbin.lt.1) then
        write(6,*)'Invalid number of bins'
        stop
      endif
      bmax=sngl(b2)
      bmin=sngl(b1)
      ...
      num4=1+(num-1)*4
      call bookup(num4,title,binsize,bmin,bmax)
      return
      end
...
c
      do i=1,50
       ...
       ides=choiceplot(i)
       if (ides.eq.1) then
         call multitop(num4,num43,n3,n2,titlex(i),titley(i),scaley(i))
       elseif (ides.eq.2) then
         call mtfill(num4,num43,n3,n2,titlex(i),titley(i),scaley(i))
       endif
      enddo
c
      return
...
c      character * (*) fname
c
      subroutine timeg77(ctime)           ! returns a string with the time
      character * (*) ctime
      return
      end  
c
...
c          stop
     ...
     # integrate(init,sig,str,n,inew,idim,icalls,avgi,sd,chi2a,isave)
      ...
      external init,sig
      common/bveg1/xl(30),xu(30),acc,ndim,ncall,itmx,nprn
      common/bveg2/xi(50,30),si,si2,swgt,schi,ndo,it
      common/newver/newver
      common/numitervegas/nitervegas
      character * (*) str
...
c create new versions of save file
      statf = newver
c      write(6,*)inew,nsteps
      
c----------------------------------------------------------
      do j=1,nsteps
c          write(*,*) 'numero Iter',j
           nitervegas=j
c           write(*,*) 'numero Iter',nitervegas 
         if(j.eq.1) then
            if(inew.eq.1) then
...
c
      ...
      subroutine suma(string,i)
      common/filessuma/files,number
      character * 25 string
      character * 25 files(20)
      files(i)=string
      number=i
      return
      end
...
c
      call mopera(j+2,'Q',j+3,j+3,dum,dum)
       ...
       common/bveg1/xl(30),xu(30),acc,ndim,ncall,itmx,nprn
       common/bveg2/xi(50,30),si,si2,swgt,schi,ndo,it
       data ncall/10000/,itmx/30/,nprn/1000/,acc/-1.d0/,
     1    xl/1.d-3, 1.d-3,1.d-3,1.d-3,1.d-3,
     #       1.d-3, 1.d-3,1.d-3,1.d-3,1.d-3, 
     #        1.d-3, 1.d-3,1.d-3,1.d-3,1.d-3,     
     #       1.d-3,1.d-3,1.d-3,1.d-3,1.d-3,
     #       1.d-3,1.d-3,1.d-3,1.d-3,1.d-3,
     #       1.d-3,1.d-3,1.d-3,1.d-3,1.d-3/,
     2   xu/1.d0,1.d0,1.d0,1.d0,1.d0,
...
c
       ...
       COMMON/SEED/NUM,NUM2
         dimension d(50,30),di(50,30),xin(50),r(50),dx(30),dt(30),
     1   x(30),kg(30),ia(30)
       dimension RAND(30)
c         
         
         common/guardar/nd,ng,npg,k,mds,ndm,dxg,dv2g,xnd,dx,
     .         xjac,dr,xin
...
c      PARAMETER (NMB=200)
c      COMMON/HISTO/HIST(NMB,100),XHIS(NMB,100),HDEL(NMB),HMIN(NMB)
c     &,HMAX(NMB),USCORE(NMB),OSCORE(NMB)
c     &,NBIN(NMB),IHIS(NMB,100),IUSCORE(NMB),IOSCORE(NMB)
c     &,IENT(NMB),HAVG(NMB),HINT(NMB),HSIG(NMB),BOOK(NMB),TITLE(NMB)
c     &,NHIST
c      COMMON/HISTO2/HIST2(10,100,100),XHIS2(10,100),YHIS2(10,100)
c     &,HDEL2(2,10),XPROJ(10,100),YPROJ(10,100),HMIN2(2,10)
c     &,HMAX2(2,10),NBIN2(2,10),IHIS2(10,100,100),IOSCORE2(10)
c     &,IENT2(10),NHIST2,HAVG2(2,10),HINT2(10),HSIG2(2,10),BOOK2(10),
c     & TITLE2(10)
c      CHARACTER TITLE*50,BOOK*3
c      CHARACTER TITLE2*50,BOOK2*3
c         write(*,*) 'paso'
c      BOOK(M)='YES'
c      NBIN(M)=NBIN2(IAX,N)
c      HDEL(M)=HDEL2(IAX,N)
c      HMIN(M)=HMIN2(IAX,N)
c      HMAX(M)=HMAX2(IAX,N)
c      NHIST=MAX(NHIST,M)
c      TITLE(M)=TITLE2(N)//'(PROJ)'
c      DO I=1,NBIN(M)
c      IF(IAX.EQ.1)      THEN
c      HIST(M,I)=XPROJ(N,I)
c      XHIS(M,I)=XHIS2(N,I)
c      ELSEIF(IAX.EQ.2)      THEN
c      HIST(M,I)=YPROJ(N,I)
c      XHIS(M,I)=YHIS2(N,I)
c      ENDIF
c      ENDDO
c      END
...
c      WRITE(99,*) CTIME(1:5)
   71 FORMAT(4X,' (   19',I2,' -',I2,' -',I2,1X,A5/)
      ...
      IFRMAX=N*M
      IFRAME=IFRAME+1
c      IF(IFRAME.GT.IFRMAX.OR.N.NE.NOLD.OR.M.NE.MOLD) THEN
      IF(IFRAME.GT.IFRMAX) THEN
        IFRAME=1
        WRITE(99,202)
...
c        WRITE(99,104) XTIT,YTIT,USCORE(NH)
104     FORMAT('  TITLE ',2(F8.4,1X),'" UFL=',1PE10.3,'"')
        YTIT=YTIT-YTIT0
      ENDIF
      IF(OSCORE(NH).NE.0.) THEN
c        WRITE(99,105) XTIT,YTIT,OSCORE(NH)
105     FORMAT('  TITLE ',2(F8.4,1X),'" OFL=',1PE10.3,'"')
        YTIT=YTIT-YTIT0
      ENDIF
c      WRITE(99,106) XTIT,YTIT,XU,YTIT,XTIT,YTIT,XTIT,YU
...
203   RETURN
      ...
      WRITE(99,202)
202   FORMAT('   NEW PLOT',/,/)
      CALL MULTITOP(NDUM,NDUM,NDUM,NDUM,' ',' ','REF')
      END                                    
