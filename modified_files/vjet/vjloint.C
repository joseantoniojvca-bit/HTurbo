...
//   double _VEV_ = sqrt(1./coupling::Gf/sqrt(2.0));
//     double alpha_qed = opts.aemmz;
//   //  double __DEB_PI = M_PI;
//   double MW = coupling::wmass;
...
// Finite Real LO contribution -- Lean -- 10/06/2024
double vjloint::calcvegasrealloint(const double x[7])
{
...
*/
  ...
  // Normalization
  double norm_off = 1.025;
  double norm_higgs = pow(M_PI, 2)/2./3. * norm_off * opts.rwidth * opts.rmass;

  double val = norm_higgs * xmsq*jac;
  ...
  if (opts.timeprofile) {
    ...
    << setw(10) << "tot time" << setw(10) << float( end_time - begin_time ) /  CLOCKS_PER_SEC << endl;
...
void vjloint::calc(const double x[5], double f[2])
{
  ...
  */
  ...
  for (int j = 0; j < 2*MAXNF+1; j++)
    ...
    */
    ...
    /*
     *   // CJU Check values of PDFs
     *   cout.precision(15);
     *   cout << endl;
     *   cout << "x1 = " << phasespace::x1 << "x2 = " << phasespace::x2 << ", muf = " << muf << endl;
     *   for (int ii = 0; ii < 2*MAXNF+1; ii++) {
     *     cout << "fx1 = " << fx1[ii] << " ";
  }
  cout << endl;
  for (int ii = 0; ii < 2*MAXNF+1; ii++) {
  ...
  */
    ...
    /*
     *   // Check values of matrix amplitude
     *   for (int ii = 0; ii < 11; ii ++) {
     *
     *     for (int jj = 0; jj < 11; jj ++) {
     *
     *         cout << "msq_omega["<< ii << "]["<< jj <<"] = " << msq_omega[ii][jj] << endl;
     *
  }
  ...
  */
    ...
    // cout << "\nx2 = " << phasespace::x2 << ", xmsq = " << xmsq << ", jac = " << jac << endl;
    ...
    f[0] = norm_higgs * xmsq*jac;
    f[1] = norm_higgs * xmsq_m*jac;
  ...
  //Drell-Yan
      ...
      */
      ...
      // calculate PDFs
      double fx1[2*MAXNF+1],fx2[2*MAXNF+1];
      pdf::fdist(1,phasespace::x1,muf,fx1);
      pdf::fdist(2,phasespace::x2,muf,fx2);
      ...
      if (opts.qed && opts.qedorder_vjet >= 1)
        {
          double ft1, ft2;
          pdf::xfxqphot(phasespace::x1,muf,ft1);
          pdf::xfxqphot(phasespace::x2,muf,ft2);
          xmsq += convolute_qed(fx1, fx2, ft1, ft2, msq_omega);
        }
    ...
    if (opts.qed && opts.qedorder_vjet >= 1)
      {
        ...
        pdf::xfxqphot(phasespace::x1,muf,ft1);
        pdf::xfxqphot(phasespace::x2,muf,ft2);
        xmsq += convolute_qed(fx1, fx2, ft1, ft2, msq);
      }
...
double vjloint::convolute(double fx1[2*MAXNF+1], double fx2[2*MAXNF+1], double msq[2*MAXNF+1][2*MAXNF+1])
{
  ...
  */
  ...
  // CJU Higgs production
  else if (opts.nproc == 4) {

    // Initialize to zero
    xmsq = 0.;

    // Convolute the PDFs with the matrix amplitude
    for (int i = 0; i < 11; i++) {

      for (int j = 0; j < 11; j++) {

        // cout << "\nmsq[" << i << "][" << j << "] = " << msq[i][j] << endl;
        xmsq += fx1[i] * fx2[j] * msq[j][i];
...
double vjloint::convolute_qed(double fx1[2*MAXNF+1], double fx2[2*MAXNF+1], double ft1, double ft2, double msq[2*MAXNF+1][2*MAXNF+1])
{
  double xmsq;
  double aqedmur = (alphaqed::calc(pow(scales::ren,2),opts.qedorder+1)*4.*M_PI); //Use alpha(mur) QED for initial state photons
  double facqedqqb = resconst::a0qed/(qcdcouple_.as_*constants::CF);
  double facqedqA  = aqedmur/(qcdcouple_.as_)*6.;
  if (opts.nproc == 1)
    {
      xmsq =fx1[0]*ft2*msq[5][0]*facqedqA*parton::chsq(parton::B);
      xmsq+=fx1[2]*ft2*msq[5][2]*facqedqA*parton::chsq(parton::S);
      xmsq+=fx1[4]*ft2*msq[5][4]*facqedqA*parton::chsq(parton::D);
      xmsq+=fx1[7]*ft2*msq[5][7]*facqedqA*parton::chsq(parton::U);
      xmsq+=fx1[9]*ft2*msq[5][9]*facqedqA*parton::chsq(parton::C);
				                                       
      xmsq+=ft1*fx2[0]*msq[0][5]*facqedqA*parton::chsq(parton::B);
      xmsq+=ft1*fx2[2]*msq[2][5]*facqedqA*parton::chsq(parton::S);
      xmsq+=ft1*fx2[4]*msq[4][5]*facqedqA*parton::chsq(parton::D);
      xmsq+=ft1*fx2[7]*msq[7][5]*facqedqA*parton::chsq(parton::U);
      xmsq+=ft1*fx2[9]*msq[9][5]*facqedqA*parton::chsq(parton::C);
				                                                                
      xmsq+=fx1[0]*fx2[7]*msq[7][0]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::B))/2.;
      xmsq+=fx1[0]*fx2[9]*msq[9][0]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::B))/2.;
      xmsq+=fx1[2]*fx2[7]*msq[7][2]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::S))/2.;
      xmsq+=fx1[2]*fx2[9]*msq[9][2]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::S))/2.;
      xmsq+=fx1[4]*fx2[7]*msq[7][4]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::D))/2.;
      xmsq+=fx1[4]*fx2[9]*msq[9][4]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::D))/2.;
      xmsq+=fx1[7]*fx2[2]*msq[2][7]*facqedqqb*(parton::chsq(parton::S)+parton::chsq(parton::U))/2.;
      xmsq+=fx1[7]*fx2[4]*msq[4][7]*facqedqqb*(parton::chsq(parton::D)+parton::chsq(parton::U))/2.;
      xmsq+=fx1[9]*fx2[0]*msq[0][9]*facqedqqb*(parton::chsq(parton::B)+parton::chsq(parton::C))/2.;
      xmsq+=fx1[9]*fx2[2]*msq[2][9]*facqedqqb*(parton::chsq(parton::S)+parton::chsq(parton::C))/2.;
      xmsq+=fx1[9]*fx2[4]*msq[4][9]*facqedqqb*(parton::chsq(parton::D)+parton::chsq(parton::C))/2.;
    }
  else if (opts.nproc == 2)
    {
      xmsq =fx1[1 ]*ft2*msq[5 ][1 ]*facqedqA*parton::chsq(parton::C);
      xmsq+=fx1[3 ]*ft2*msq[5 ][3 ]*facqedqA*parton::chsq(parton::U);
      xmsq+=fx1[6 ]*ft2*msq[5 ][6 ]*facqedqA*parton::chsq(parton::D);
      xmsq+=fx1[8 ]*ft2*msq[5 ][8 ]*facqedqA*parton::chsq(parton::S);
      xmsq+=fx1[10]*ft2*msq[5 ][10]*facqedqA*parton::chsq(parton::B);

      xmsq+=ft1*fx2[1 ]*msq[1 ][5 ]*facqedqA*parton::chsq(parton::C);
      xmsq+=ft1*fx2[3 ]*msq[3 ][5 ]*facqedqA*parton::chsq(parton::U);
      xmsq+=ft1*fx2[6 ]*msq[6 ][5 ]*facqedqA*parton::chsq(parton::D);
      xmsq+=ft1*fx2[8 ]*msq[8 ][5 ]*facqedqA*parton::chsq(parton::S);
      xmsq+=ft1*fx2[10]*msq[10][5 ]*facqedqA*parton::chsq(parton::B);
      
      xmsq+=fx1[1 ]*fx2[6 ]*msq[6 ][1 ]*facqedqqb*(parton::chsq(parton::D)+parton::chsq(parton::C))/2.;
      xmsq+=fx1[1 ]*fx2[8 ]*msq[8 ][1 ]*facqedqqb*(parton::chsq(parton::S)+parton::chsq(parton::C))/2.;
      xmsq+=fx1[1 ]*fx2[10]*msq[10][1 ]*facqedqqb*(parton::chsq(parton::B)+parton::chsq(parton::C))/2.;
      xmsq+=fx1[3 ]*fx2[6 ]*msq[6 ][3 ]*facqedqqb*(parton::chsq(parton::D)+parton::chsq(parton::U))/2.;
      xmsq+=fx1[3 ]*fx2[8 ]*msq[8 ][3 ]*facqedqqb*(parton::chsq(parton::S)+parton::chsq(parton::U))/2.;
      xmsq+=fx1[3 ]*fx2[10]*msq[10][3 ]*facqedqqb*(parton::chsq(parton::B)+parton::chsq(parton::U))/2.;
      xmsq+=fx1[6 ]*fx2[1 ]*msq[1 ][6 ]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::D))/2.;
      xmsq+=fx1[6 ]*fx2[3 ]*msq[3 ][6 ]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::D))/2.;
      xmsq+=fx1[8 ]*fx2[1 ]*msq[1 ][8 ]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::S))/2.;
      xmsq+=fx1[8 ]*fx2[3 ]*msq[3 ][8 ]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::S))/2.;
      xmsq+=fx1[10]*fx2[1 ]*msq[1 ][10]*facqedqqb*(parton::chsq(parton::C)+parton::chsq(parton::B))/2.;
      xmsq+=fx1[10]*fx2[3 ]*msq[3 ][10]*facqedqqb*(parton::chsq(parton::U)+parton::chsq(parton::B))/2.;
    }
  else if (opts.nproc == 3)
    {
      //qA
      ...
      //qqb
      xmsq+=fx1[0 ]*fx2[10]*msq[10][0 ]*facqedqqb*parton::chsq(parton::B);
      xmsq+=fx1[1 ]*fx2[9 ]*msq[9 ][1 ]*facqedqqb*parton::chsq(parton::C);
      xmsq+=fx1[2 ]*fx2[8 ]*msq[8 ][2 ]*facqedqqb*parton::chsq(parton::S);
      xmsq+=fx1[3 ]*fx2[7 ]*msq[7 ][3 ]*facqedqqb*parton::chsq(parton::U);
      xmsq+=fx1[4 ]*fx2[6 ]*msq[6 ][4 ]*facqedqqb*parton::chsq(parton::D);
      xmsq+=fx1[6 ]*fx2[4 ]*msq[4 ][6 ]*facqedqqb*parton::chsq(parton::D);
      xmsq+=fx1[7 ]*fx2[3 ]*msq[3 ][7 ]*facqedqqb*parton::chsq(parton::U);
      xmsq+=fx1[8 ]*fx2[2 ]*msq[2 ][8 ]*facqedqqb*parton::chsq(parton::S);
      xmsq+=fx1[9 ]*fx2[1 ]*msq[1 ][9 ]*facqedqqb*parton::chsq(parton::C);
      xmsq+=fx1[10]*fx2[0 ]*msq[0 ][10]*facqedqqb*parton::chsq(parton::B);
    }
...
//fortran interface
double convolute_qed_(double fx1[2*MAXNF+1], double fx2[2*MAXNF+1], double &ft1, double &ft2, double msq[2*MAXNF+1][2*MAXNF+1])
{
  if (opts.qed && opts.qedorder_vjet >= 1)
    return vjloint::convolute_qed(fx1, fx2, ft1, ft2, msq);
