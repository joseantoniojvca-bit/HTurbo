...
void evolnum::calculate()
{
  ...
  double facscale = abs(pdfevol::mubstartilde);  //mubstartilde = mubstar * Q / sqrt((pow(mubstar,2) + pow(Q,2))), with Q = mures or mufac
  
  mellinpdf::allocate();
  //mellinpdf::evalpdfs(facscale);
  mellinpdf::evalpdfs(facscale,phasespace::m,phasespace::y);
  if (opts.gjxnspace && opts.evolmode == 2)
    mellinpdf::gjx(pdfevol::bbb);
