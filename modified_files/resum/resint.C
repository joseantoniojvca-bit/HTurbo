...
{
  //  cout << " x " << x << " besselint_dequad " << real(besselint::bint(x)) << endl;
  return real(besselint::bint(x));
}
/*
double besselint_fbt(double &x)
{
  ...
  if (resint::_mode == 3 || resint::_mode == 4)
    ...
    JN = fort_besj0_(qtb);
  //cout << " x " << x << " besselint_fbt " << real(besselint::bint(x))/fort_besj0_(qtb) << endl;
  return real(besselint::bint(x))/JN;
}
*/

double besselint_mp_real_fbt(double &x)
{
  //cout << " fbt " << x << " " << besselint::bint(x) << endl; 
  return real(besselint::bint(x));
}
/*
double besselint_mp_complex_fbt(double &x)
{
  *********** Butterfly contour **************
  ...
  **********************************************
  ...
  //Call complex-argument bessel
  //fcomplex cbj[2], cdj[2], cby[2], cdy[2];
  //cjynb_ ( n, qtbu, nm, cbj, cdj, cby, cdy );
  //h1 = cx(cbj[nn]);
  ...
  return real(bint_up*jacu*h1)/JN; //<--
}
*/
...
/*
double besselint_mp_real_hankel(double x)
{
  //cout << " hankel " << x << " " << besselint::bint(x) << endl; 
  return real(besselint::bint(x))/x;
}
*/
...
void resint::rint(double costh, double m, double qt, double y, int mode, double f[2])
{
  ...
  */
  ...
  //aaqed is alphaqed/pi at the renormalization scale
  aaqed = alphaqed::calc(pow(scales::ren,2),opts.qedorder+1)*4.;

  if (opts.alpha0)
    aaqed = resconst::a0qed/M_PI;
...
//  //  cout << q << "  " << mur << "  " << blimit_.rblim_ << "  " << a_param_.b0p_/lambdaqcd << "  " << lambdaqcd <<"  " <<  a_param_.b0p_/blim << endl;
  ...
  if (!(opts.order == 0 && opts.xspace))
	...
	mesq::setmesq(1., costh, pow(costh,2));
      else //costh integrated, y-differential
	{
  ...
  if(opts.calc_type == "OLD")
    ...
    hcoeff::calc(m*m,costh);



  //  cout << " hcoef from resint.C " << endl;
...
double resint::bintegral(double qt)
{
  ...
  if (opts.bprescription == 0 || opts.bprescription == 4)
    {
      ...
      double Q = 2.;
      int nu = 1;//(mode == 3 || mode == 4)?1:0;
      double z = 1.;
      int n = 50;
      fbt_(besselint_fbt, qt, Q, nu, z, n, res);
      err = 0.;
      res *= 2*M_PI;
      cout << "fbt    result of inverse bessel transform, pt=" << _qt << " m=" << _m << " y=" << _y << " : " << setprecision(16) << res << " +- " << err << endl;
  ...
  //Minimal prescription to avoid Landau singularity (see hep-ph/9604351 and hep-ph/0002078)
	...
	cout << "warning: dequad abnormal termination, pt=" << _qt << " m=" << _m << " y=" << _y << " bint: " << resdequad << " err " << errdequad << endl;
      ...
      double Q = 2.;
      int nu = (resint::_mode == 3 || resint::_mode == 4)?1:0;
      double z = 1.;
      int n = 30;
      fbt_(besselint_mp_complex_fbt, qt, Q, nu, z, n, res);
      err = 0.;
      res *= 2*M_PI;
      //cout << "fbt    result of inverse bessel transform, pt=" << _qt << " m=" << _m << " y=" << _y << " : " << setprecision(16) << res << " +- " << err << endl;
      */
  ...
  //Minimal prescription along the real axis, crossing the Landau singularity
      ...
      if (opts.bcf > 0)
	{
      ...
      */
      ...
      //double z = 1.;
      //int n = 20;
      //fbt_(besselint_mp_real_fbt, qt, Q, nu, z, n, res);
      //err = 0.;
      //res *= 2*M_PI;
      //cout << "fbt    result of inverse bessel transform, pt=" << _qt << " m=" << _m << " y=" << _y << " : " << setprecision(16) << res << " +- " << err << endl;
      ...
      /*
      double h;
      h = get_ht_(besselint_mp_real_fbt, nu, n, qt, Q);
      //cout << h << endl;
      if (h < 0)
	{
	  //n *= 2;
	  h = fabs(get_ht_(besselint_mp_real_fbt, nu, n, qt, Q)); ///2.;
	  h = fabs(h);
	  hankel::init(nu,n,h);
	  hankel::transform(besselint_mp_real_hankel, qt, res, err);
	  hankel::free();
      ...
      else
	{
      ...
      //cout << "hankel:: result of inverse bessel transform, pt=" << _qt << " m=" << _m << " y=" << _y << " : " << setprecision(16) << res << " +- " << err << endl;
      */
    }
