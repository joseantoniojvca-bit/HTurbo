...
complex <double> evolnative::Sqed;
...
complex <double> *evolnative::ansqedup;
complex <double> *evolnative::ansqeddn;
complex <double> *evolnative::ansqedft;
...
void evolnative::allocate_engine()
{
  ...
  ansqedup   = new complex <double> [ndim*2];
  ansqeddn   = new complex <double> [ndim*2];
  ansqedft   = new complex <double> [ndim*2];
...
void anom(
	  ...
	  complex <double> &rgq,
	  complex <double> &rgg,
	  complex <double> &ansqedup,
	  complex <double> &ansqeddn,
	  complex <double> &ansqedft
	  )
{
...
//     1         1.532D0 / (XN+4.) + 0.839D0 / (XN+5.)
  ...
  complex <double> QQqed=(3./4.)*QQI; //Removemos el factor CF, es proporcional a la carga al cuadrado
  ...
  complex <double> AAqed= 2. * GGF * alphaqed::NF2; //Solo sobrevive la parte proporcional a la suma de cargas             
  ansqedup = alphaqed::equarkup*QQqed / (2.*B0QED);
  ansqeddn = alphaqed::equarkdn*QQqed / (2.*B0QED);
  ansqedft = AAqed / (2.*B0QED);
}
...
//  else
	...
	FOP[k+mellinint::mdim] = mellinpdf::FT_2[k];
  ...
  for (int i = 0; i < ndim*2; i++)
    {
      ...
      complex <double> Ansqedup, Ansqeddn, Ansqedft;
      anom(cxn[i], Ans, Am, Ap, Al, Be, Ab, Rmin, Rplus, Rqq, Rqg, Rgq, Rgg, Ansqedup, Ansqeddn, Ansqedft);
...
void evolnative::scales()
{
  ...
  */
  //Force alpqf in msbar
  if (opts.alphaslha)
    alpqf = pdf::alphas(scales::res)/4./M_PI;
  else
    alpqf = pdf::rgktalphas(scales::res)/4./M_PI;
...
//    }
  ...
  for (int sign = mesq::positive; sign <= mesq::negative; sign++)
	...
	complex <double> UVN,DVN,SVN,CVN,BVN,USN,DSN,SSN,CHN,BON,GLN,SIN,NS3N,NS8N,NS15N,NS24N,NS35N,FON;
	if (sign == mesq::positive)
	  {
	    UVN   = UVP[n];
	    DVN   = DVP[n];
	...
	complex <double> ANSqedup = ansqedup[i];
	complex <double> ANSqeddn = ansqeddn[i];
	complex <double> ANSqedft = ansqedft[i];
	// **************************************
	...
	//QED
	if (opts.qed && opts.qedorder_evol >= 1 && pdf::photon) //Multiplicative QED evolution: first QED and then QCD
	  {
	    ...
	    complex <double> NST1up = UVN + 2.*USN - CVN - 2.*CHN; // u-c
	    complex <double> NST1dn = DVN + 2.*DSN - SVN - 2.*SSN; // d-s
	    complex <double> NST3dn = DVN + 2.*DSN + SVN + 2.*SSN - 2.* BVN - 4.*BON; //  d+s-2b
	    complex <double> SGup   = UVN + 2.*USN + CVN + 2.*CHN; // u+c
	    complex <double> SGdn   = DVN + 2.*DSN + SVN + 2.*SSN + BVN + 2.*BON; // d+s+b
	    ...
	    NS3N  = (-3.*NST1dn + 3.*NST1up - NST3dn - 2.*SGdn + 3.*SGup)/6.;
	    NS8N  = ( 9.*NST1dn + 3.*NST1up - NST3dn - 2.*SGdn + 3.*SGup)/6. ;
	    NS15N = ( 6.*NST1up + NST3dn + 2.*SGdn - 3.*SGup)/3.;
	    NS24N = ( 5.*NST3dn)/3. - (2.*SGdn)/3. + SGup;
	    SG    = SGup+SGdn;
	    NS35N = SG;
	  }
	...
	if (opts.order_evol == 1)
	  ...
	  {
	    UVN   *= ENS * (1.+  alpr * XL1 * RMIN);
	    DVN   *= ENS * (1.+  alpr * XL1 * RMIN);
	    SVN   *= ENS * (1.+  alpr * XL1 * RMIN);
	    CVN   *= ENS * (1.+  alpr * XL1 * RMIN);
	    BVN   *= ENS * (1.+  alpr * XL1 * RMIN);
	    NS3N  *= ENS * (1.+  alpr * XL1 * RPLUS);
	    NS8N  *= ENS * (1.+  alpr * XL1 * RPLUS);
	    NS15N *= ENS * (1.+  alpr * XL1 * RPLUS);
	    NS24N *= ENS * (1.+  alpr * XL1 * RPLUS);
	...
	SSN = (10.* SIN + 2.* NS35N + 3.* NS24N + 5.* NS15N - 20.* NS8N - 60.* SVN) / 120.;
	...
	CHN = (10.* SIN + 2. *NS35N + 3.* NS24N - 15.* NS15N - 60.* CVN) / 120.;
	BON = (10.* SIN + 2. *NS35N - 12.* NS24N - 60.* BVN) / 120.;
