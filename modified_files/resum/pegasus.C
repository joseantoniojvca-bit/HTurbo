...
//	  cxn = mellinint::Np_2[i-mellinint::mdim];
  ...
  if (order_.npord_ > 0 || opts.order_evol >= 2 || opts.order_hcoef >= 2)
    {
      pns1mom_();
      psg1mom_();
      usg1mom_();
    }
  if (itord_.nuord_ > 1 && evmod_.imodev_ < 3) usg1hmom_();

  if (order_.npord_ > 1 || opts.order_evol >= 3 || opts.order_hcoef >= 3)
    {
      pns2mom_();
      psg2mom_();
      uns2mom_();
  ...
  if (itord_.nuord_ > 2 && evmod_.imodev_ < 3) usg2hmom_();
...
//	  asfthr_.asc_ = pdf::extalphas(sqrt(asfthr_.m2c_)) /(4.*M_PI);
//	  asfthr_.asb_ = pdf::extalphas(sqrt(asfthr_.m2b_)) /(4.*M_PI);
//	  asfthr_.ast_ = pdf::extalphas(sqrt(asfthr_.m2t_)) /(4.*M_PI);
...
//	}
  ...
  //cout << "m0 " << asinp_.m20_  << "  " << asinp_.as0_  *4.*M_PI<< "  " << pdf::extalphas(sqrt(asinp_.m20_)) << endl;
  //cout << "mc " << asfthr_.m2c_ << "  " << asfthr_.asc_ *4.*M_PI<< "  " << pdf::extalphas(sqrt(asfthr_.m2c_)) << endl;
  //cout << "mb " << asfthr_.m2b_ << "  " << asfthr_.asb_ *4.*M_PI<< "  " << pdf::extalphas(sqrt(asfthr_.m2b_)) << endl;
  //cout << "mt " << asfthr_.m2t_ << "  " << asfthr_.ast_ *4.*M_PI<< "  " << pdf::extalphas(sqrt(asfthr_.m2t_)) << endl;
...
void pegasus::init()
{
  ...
  else if (opts.asrun == 5)
    {
      ivfns = 1; //VFN evolution (read from LHAPDF)
      nff = 4;   //in FFN evolution, number of flavours  --> should read from LHAPDF
      order_.npord_ = opts.order_sudak; //order of evolution read from LHAPDF
      evmod_.imodev_ = 1; //reproduces evolution in x-space --> actually for NNPDF options 3 or 4 may be better
    }
...
void pegasus::evolve()
{
  if (opts.evolmode == 3 || opts.evolmode == 4)  //<- check this for the commutator
  //always retrieve stored PDFs
  retrieve();
  ...
  */
  ...
  if (opts.evolmode == 1)
    {
      ...
      asi = resint::alpqres; //! <- this one is correct
      // if (opts.commute)
      //asi = resint::alpqfac;
  ...
  //Normal Pegasus evolution: VFNS evolution from the starting scale upward
  else if (opts.evolmode == 3)
    {
      ...
      //cout << pdf::extalphas(sqrt(asinp_.m20_))/(4.* M_PI) << "  " << alphas(asinp_.m20_, asinp_.m20_, asi, NF) << endl;

      //asf = alphas(pdfevol::mubstartilde, asi, NF);  // --> evolve alphas with Pegasus Runge-Kutta (allow complex scales)
      //asf = pdf::extalphas(sqrt(M2))/(4.* M_PI);        // --> evolve alphas with the LHAPDF interpolation
  ...
  //if (false)
  //if (opts.commute && (opts.kmufac != opts.kmures))
  if (opts.evolmode == 3 && (opts.kmufac != opts.kmures))
    {
      ...
      */
      ...
      //Fixed flavour evolution from Q to muF
      //order_.npord_ = max(2,opts.order_evol-1);
      order_.npord_ = opts.order_evol-1;
...
complex <double> pegasus::alphas(complex <double> m, double &ASI, int &NF)
{
  ...
  if (M2 > asfthr_.m2t_) //If M2 = M2tilde than the scale is frozen at muf, and never goes above the top
  ...
  else if (M2 > asfthr_.m2c_ || asinp_.m20_ > (asfthr_.m2c_-1e-3))   //if the starting scale is above mcharm never switch to 3 flavour (intrinsic charm)
    {
      ...
      alphas::expan(r, sqrt(R2C), asfthr_.asc_*4*M_PI, NF);
      if (aspar_.naord_ == 0)
  ...
  else
    {
      ...
      alphas::expan(r, sqrt(R20), asinp_.as0_*4*M_PI, NF);
      if (aspar_.naord_ == 0)
	ASF = alphas::asLO/4./M_PI;
      else if (aspar_.naord_ == 1)
	ASF = alphas::asNLO/4./M_PI;
