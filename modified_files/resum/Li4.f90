...
! Li_4(x) for x in [-1,0]
double precision function ldli4_neg(x)
  implicit none
...
end function ldli4_neg


! Li_4(x) for x in [0,1/2]
double precision function ldli4_half(x)
  implicit none
  double precision :: x, x2, x4, p, q
  double precision, parameter :: cp(6) = (/                &
...
end function ldli4_half


! Li_4(x) for x in [1/2,8/10]
double precision function ldli4_mid(x)
  implicit none
  double precision :: x, x2, x4, p, q
...
end function ldli4_mid


! Li_4(x) for x in [8/10,1]
double precision function ldli4_one(x)
  implicit none
  ...
  ldli4_one = zeta4 + l*(zeta3 + l*(0.5D0*zeta2 + l*(11.0D0/36 &
     - 1.0D0/6*log(-l) + l*(-1.0D0/48 + l*(-1.0D0/1440        &
     + l2*(1.0D0/604800 - 1.0D0/91445760*l2))))))

end function ldli4_one
...
!*********************************************************************
double precision function ldli4(x)
  implicit none
  double precision :: x, app, rest, sgn, l, l2
  double precision :: ldli4_neg, ldli4_half, ldli4_mid, ldli4_one
  double precision, parameter :: zeta2 = 1.6449340668482264D0
  double precision, parameter :: zeta4 = 1.0823232337111382D0
...
!> @author Alexander Voigt
!*********************************************************************
double complex function cldli4(z)
  implicit none
  double complex :: z, u, u2, u4, u8, c3, lmz, r, pos_cdlog
  double precision :: rz, iz, nz, pz, lnz, arg, sgn, ldli4
  ...
  iz = aimag(z)
   ...
   if (iz .eq. 0) then
      if (rz .le. 1) then
         cldli4 = dcmplx(ldli4(rz), iz)
         return
      else
         lnz = log(rz)
         cldli4 = dcmplx(ldli4(rz), -1D0/6D0*PI*lnz**3)
         return
      endif
   endif
