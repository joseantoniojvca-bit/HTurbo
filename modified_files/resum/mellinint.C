...
complex <double> mellinint::GQN_1;
complex <double> mellinint::GQN_2;
// ANDREA: photon for W 
complex <double> mellinint::QAN_1;
complex <double> mellinint::QAN_2;
//
complex <double> mellinint::QGNUP_1;
complex <double> mellinint::QGNDW_2;
complex <double> mellinint::QGNDW_1;
complex <double> mellinint::QGNUP_2;
complex <double> mellinint::QANUP_1;
complex <double> mellinint::QANDW_2;
complex <double> mellinint::QANDW_1;
complex <double> mellinint::QANUP_2;
complex <double> mellinint::QQBNUP;
complex <double> mellinint::QQBNDW;
...
complex <double> mellinint::QBQN;
complex <double> mellinint::QBQPN_2;
complex <double> mellinint::QBQPN_1;
complex <double> mellinint::QBQBPN_2;
complex <double> mellinint::QBQBPN_1;
complex <double> mellinint::QPQPN_2;
complex <double> mellinint::QPQPN_1;
complex <double> mellinint::QPQBPN;
complex <double> mellinint::QBPQPN;

complex <double> mellinint::GGN_bb;

//HF
complex <double> mellinint::BBRN;
complex <double> mellinint::CCBN;
complex <double> mellinint::bbfun;
complex <double> mellinint::ccfun;
  ...
  if (opts.mellin1d)
    {
      ...
      //allocate also N_1 and N_2 to allow separate beams
...
//	}
  ...
  if (opts.mellin1d)
    for (int i=0; i < opts.mellinintervals; i++)
      for (int j=0; j < opts.mellinrule; j ++)
	{
	  Np_1[j+i*opts.mellinrule] = Np[j+i*opts.mellinrule];
	  Np_2[j+i*opts.mellinrule] = Np[j+i*opts.mellinrule];
	}
...
void mellinint::pdf_mesq_expy(int i1, int i2, int sign)
{
	...
	if (opts.nproc == 4) {
		...
		// gg -> h matrix element
		complex <double> mesq_gg = mesq::mesqij_expy[mesq::index(0,i1,i2,sign)];

		//LL part
		GGN = fn1[g]*fn2[g] * mesq_gg;
	...
	if (opts.nproc >= 3)
	{
		...
		QQBNUP = fn1[u ]*fn2[ub]*mesq_uub
		+fn1[c ]*fn2[cb]*mesq_ccb
		+fn1[ub]*fn2[u ]*mesq_ubu
		+fn1[cb]*fn2[c ]*mesq_cbc;

		QQBNDW = fn1[d ]*fn2[db]*mesq_ddb
		+fn1[s ]*fn2[sb]*mesq_ssb
		+fn1[b ]*fn2[bb]*mesq_bbr
		+fn1[db]*fn2[d ]*mesq_dbd
		+fn1[sb]*fn2[s ]*mesq_sbs
		...
		BBRN = fn1[b ]*fn2[bb]*mesq_bbr
		+fn1[bb]*fn2[b ]*mesq_brb;

		CCBN = fn1[c ]*fn2[cb]*mesq_ccb
		+fn1[cb]*fn2[c ]*mesq_cbc;
...
/*
 c o*mplex <double> mellinint::integrand()
 {
 if (opts.order == 0)
	 return QQBN;
  ...
  for (int i = 0; i < mdim; i++)
    {
      ...
      complex <double> bbpos = 0.;
      complex <double> ccpos = 0.;
      ...
      //contribution starting at LL
      pos +=
	+ QQBN             *(hcoeff::Hqqb[i]*expc::qqb[idx]+muf::qqb[idx]);
	...
	pos +=
	  +(QGN_1+QGN_2)  *(hcoeff::Hqg[i]    *expc::qg[idx]   + muf::qg[idx]  );
    
      //contributions starting at NNLL
      if (opts.order_hcoef >= 2)
	pos +=
	  + GGN              *(hcoeff::Hgg[i]  *expc::gg[idx]   + muf::gg[idx]  )
	  + GGN_bb           *(hcoeff::Hgg[i]  *expc::gg_bb[idx]+ muf::gg[idx]  )
	  + (QQN_1+QQN_2)    *(hcoeff::Hqq[i]  *expc::qq[idx]   + muf::qq[idx]  )
	  + (QQPN_1+QQPN_2)  *(hcoeff::Hqqp[i] *expc::qqp[idx]  + muf::qqp[idx] )
	  + (QQBPN_1+QQBPN_2)*(hcoeff::Hqqbp[i]*expc::qqbp[idx] + muf::qqbp[idx]);

      //contributions starting at NNNLL
      if (opts.order_hcoef >= 3)
	pos +=
	  + QQBN_nfz         *hcoeff::Hqqb_nfz *expc::qqb[idx]
	  + (QBGN_1+QBGN_2)  *(hcoeff::Hqbg[i] *expc::qbg[idx]  + muf::qbg[idx] )
	  + (QPGN_1+QPGN_2)  *(hcoeff::Hqpg[i] *expc::qpg[idx]  + muf::qpg[idx] )
	  + (QBPGN_1+QBPGN_2)*(hcoeff::Hqbpg[i]*expc::qbpg[idx] + muf::qbpg[idx]);

      //contributions starting at NNNNLL
      if (opts.order_hcoef >= 4)
	pos +=
	  +QBQN               *(hcoeff::Hqbq[i]  *expc::qbq[idx]   + muf::qbq[idx]   )
	  +QPQBPN             *(hcoeff::Hqpqbp[i]*expc::qpqbp[idx] + muf::qpqbp[idx] )
	  +QBPQPN             *(hcoeff::Hqbpqp[i]*expc::qbpqp[idx] + muf::qbpqp[idx] )
	  +(QBQPN_1+QBQPN_2)  *(hcoeff::Hqbqp[i] *expc::qbqp[idx]  + muf::qbqp[idx]  )
	  +(QBQBPN_1+QBQBPN_2)*(hcoeff::Hqbqbp[i]*expc::qbqbp[idx] + muf::qbqbp[idx] )
	  +(QPQPN_1+QPQPN_2)  *(hcoeff::Hqpqp[i] *expc::qpqp[idx]  + muf::qpqp[idx]  );
...
//      	   << endl;
      ...
      complex <double> bbneg = 0.;
      complex <double> ccneg = 0.;
      idx = anomalous::index(i,mesq::negative);
      pdfevol::retrieve1d_neg();
      //pdfevol::retrieve1d_pos(i+mellinint::mdim); //!!!should retrieve as pdfevol::retrieve1d_pos(i+mellinint::mdim)
      pdf_mesq_expy(i,i,mesq::negative);

      //contribution starting at LL
      neg +=
	QQBN               *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
      ...
      if (opts.order_hcoef >= 1)
	neg +=
	  + (QGN_1+QGN_2)    *(conj(hcoeff::Hqg[i]  ) *expc::qg[idx]   + muf::qg[idx]);

      //contributions starting at NNLL
      if (opts.order_hcoef >= 2)
	neg +=
	  + GGN              *(conj(hcoeff::Hgg[i]  )*expc::gg[idx]    + muf::gg[idx]  )
	  + GGN_bb           *(conj(hcoeff::Hgg[i]  )*expc::gg_bb[idx] + muf::gg[idx]  )
	  + (QQN_1+QQN_2)    *(conj(hcoeff::Hqq[i]  )*expc::qq[idx]    + muf::qq[idx]  )
	  + (QQPN_1+QQPN_2)  *(conj(hcoeff::Hqqp[i] )*expc::qqp[idx]   + muf::qqp[idx] )
	  + (QQBPN_1+QQBPN_2)*(conj(hcoeff::Hqqbp[i])*expc::qqbp[idx]  + muf::qqbp[idx]);
	    
      //contributions starting at NNNLL
      if (opts.order_hcoef >= 3)
	neg +=
	  + QQBN_nfz         * conj(hcoeff::Hqqb_nfz)*expc::qqb[idx]
	  + (QBGN_1+QBGN_2)  *(conj(hcoeff::Hqbg[i] )*expc::qbg[idx]   + muf::qbg[idx] )
	  + (QPGN_1+QPGN_2)  *(conj(hcoeff::Hqpg[i] )*expc::qpg[idx]   + muf::qpg[idx] )
	  + (QBPGN_1+QBPGN_2)*(conj(hcoeff::Hqbpg[i])*expc::qbpg[idx]  + muf::qbpg[idx]);

      //contributions starting at NNNNLL
      if (opts.order_hcoef >= 4)
	neg +=
	  +QBQN               *(conj(hcoeff::Hqbq[i]  )*expc::qbq[idx]  + muf::qbq[idx]   )
	  +QPQBPN             *(conj(hcoeff::Hqpqbp[i])*expc::qpqbp[idx]+ muf::qpqbp[idx] )
	  +QBPQPN             *(conj(hcoeff::Hqbpqp[i])*expc::qbpqp[idx]+ muf::qbpqp[idx] )
	  +(QBQPN_1+QBQPN_2)  *(conj(hcoeff::Hqbqp[i] )*expc::qbqp[idx] + muf::qbqp[idx]  )
	  +(QBQBPN_1+QBQBPN_2)*(conj(hcoeff::Hqbqbp[i])*expc::qbqbp[idx]+ muf::qbqbp[idx] )
	  +(QPQPN_1+QPQPN_2)  *(conj(hcoeff::Hqpqp[i] )*expc::qpqp[idx] + muf::qpqp[idx]  );
  ...
  if (opts.bprescription == 0 || opts.bprescription == 3 || opts.bprescription == 4)
    {
      bbfun = real(bbfun/2.);
      ccfun = real(ccfun/2.);
      return real(fun/2.);
...
{// cout << " calc2d mellinint.C " << endl;
  ...
  for (int i1 = 0; i1 < mdim; i1++)
      ...
      for (int i2 = 0; i2 < mdim; i2++)
	{
	  ...
	  //contribution starting at LL
	  fun +=
	    QQBN*(hcoeff::Hqqb[ii]*expc::qqb[ii]+muf::qqb[ii]);
	  ...
	  if (opts.order_hcoef >= 1)
	    fun +=
	      + QGN_1  *(hcoeff::Hqg_1[ii]*expc::qg_1[ii]    +muf::qg_1[ii]  ) + QGN_2  *(hcoeff::Hqg_2[ii]  *expc::qg_2[ii]  +muf::qg_2[ii]);

	  //contributions starting at NNLL
	  if (opts.order_hcoef >= 2)
	    fun +=
	      + GGN    *(hcoeff::Hgg[ii]    *expc::gg[ii]    +muf::gg[ii]    )
	      + GGN_bb *(hcoeff::Hgg[ii]    *expc::gg_bb[ii] +muf::gg[ii]    )
	      + QQN_1  *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  ) + QQN_2  *(hcoeff::Hqq_2[ii]  *expc::qq_2[ii]  +muf::qq_2[ii]  )
	      + QQPN_1 *(hcoeff::Hqqp_1[ii] *expc::qqp_1[ii] +muf::qqp_1[ii] ) + QQPN_2 *(hcoeff::Hqqp_2[ii] *expc::qqp_2[ii] +muf::qqp_2[ii] )
	      + QQBPN_1*(hcoeff::Hqqbp_1[ii]*expc::qqbp_1[ii]+muf::qqbp_1[ii]) + QQBPN_2*(hcoeff::Hqqbp_2[ii]*expc::qqbp_2[ii]+muf::qqbp_2[ii]);

	  //contributions starting at NNNLL
	  if (opts.order_hcoef >= 3)
	    fun +=
	      + QQBN_nfz*hcoeff::Hqqb_nfz*expc::qqb[ii]
	      + QBGN_1  *(hcoeff::Hqbg_1[ii]*expc::qbg_1[ii] +muf::qbg_1[ii])+QBGN_2  *(hcoeff::Hqbg_2[ii] *expc::qbg_2[ii] +muf::qbg_2[ii] )
	      + QPGN_1  *(hcoeff::Hqbg_1[ii]*expc::qpg_1[ii] +muf::qbg_1[ii])+QPGN_2  *(hcoeff::Hqpg_2[ii] *expc::qpg_2[ii] +muf::qpg_2[ii] )
	      + QBPGN_1 *(hcoeff::Hqbg_1[ii]*expc::qbpg_1[ii]+muf::qbg_1[ii])+QBPGN_2 *(hcoeff::Hqbpg_2[ii]*expc::qbpg_2[ii]+muf::qbpg_2[ii])
	      ;

	  //contributions starting at NNNNLL
	  if (opts.order_hcoef >= 4)
	    fun +=
	      +QBQN               *(hcoeff::Hqbq[ii]  *expc::qbq[ii]  )
	      +QPQBPN             *(hcoeff::Hqpqbp[ii]*expc::qpqbp[ii])
	      +QBPQPN             *(hcoeff::Hqbpqp[ii]*expc::qbpqp[ii])
	      +QBQPN_1  *(hcoeff::Hqbqp_1[ii] *expc::qbqp_1[ii] ) + QBQPN_2   *(hcoeff::Hqbqp_2[ii] *expc::qbqp_2[ii] )
	      +QBQBPN_1 *(hcoeff::Hqbqbp_1[ii]*expc::qbqbp_1[ii]) + QBQBPN_2  *(hcoeff::Hqbqbp_2[ii]*expc::qbqbp_2[ii])
	      +QPQPN_1  *(hcoeff::Hqpqp_1[ii] *expc::qpqp_1[ii] ) + QPQPN_2   *(hcoeff::Hqpqp_2[ii] *expc::qpqp_2[ii] );
...
// QED contributions
// --------------------------------------------------------------------------
complex <double> mellinint::calc1d(double eq2, std::string sector)
{
  complex <double> fun = 0.;
  int idx;
  for (int i = 0; i < mdim; i++)
    {
      //Positive branch
      // Factor 1/2 in QCD part is to avoid double counting with the QED
      // prcedure separating in up/down
      // Eanch QED term has to be WITHOUT 1/2!
      complex <double> pos = 0.;
      idx = anomalous::index(i,mesq::positive);
      pdfevol::retrieve1d_pos(i);
      pdf_mesq_expy(i,i,mesq::positive);
      //contribution starting at LL      
      //	+ QQBN             *(hcoeff::Hqqb[i]*expc::qqb[idx]+muf::qqb[idx]);
      // The line before is the original line
      if(sector == "up" || sector== "UP" || sector== "Up")
	{
	  pos += + QQBNUP             *(hcoeff::Hqqb[i]*expc::qqb[idx]+muf::qqb[idx]);
	  pos += + QQBNUP*             eq2*(hcoeff::Hqqb_qed[i]*expc::qqbqed[idx]+muf::qqbqed[idx]);
	}
      else if (sector== "down" || sector == "DOWN" || sector == "Down") 
	{
	  pos += + QQBNDW             *(hcoeff::Hqqb[i]*expc::qqb[idx]+muf::qqb[idx]);
	  pos += + QQBNDW*             eq2*(hcoeff::Hqqb_qed[i]*expc::qqbqed[idx]+muf::qqbqed[idx]);
	}
      else
	  {
	    cout << " wrong quark sector member in mellinint.C " << endl;
	    cout << " Available options up, UP, Up, down, DOWN, Down " << endl;
	  }     
      //contribution starting at NLL
      if (opts.order_hcoef >= 1)
	{
	  if(sector == "up" || sector== "UP" || sector== "Up")
	    {
	      pos +=
		+(QGNUP_1+QGNUP_2)  *(hcoeff::Hqg[i]    *expc::qg[idx]   + muf::qg[idx]  );
	      if(opts.qedorder_hcoef >=1) pos +=+(QANUP_1+QANUP_2)*eq2  *(hcoeff::Hqgamma[i]    *expc::qgamma[idx]   + muf::qgamma[idx]  );
	    }
	  else if (sector== "down" || sector == "DOWN" || sector == "Down")  
	    {
	      pos +=+(QGNDW_1+QGNDW_2)  *(hcoeff::Hqg[i]    *expc::qg[idx]   + muf::qg[idx]  );
	      if(opts.qedorder_hcoef >=1) pos += +(QANDW_1+QANDW_2) *eq2 *(hcoeff::Hqgamma[i]    *expc::qgamma[idx]   + muf::qgamma[idx]  );
	    }
	  else
	    {
	      cout << " wrong quark sector member in mellinint.C " << endl;
      ...
      if (opts.order_hcoef >= 2)
	pos +=
	  + GGN/2.              *(hcoeff::Hgg[i]  *expc::gg[idx]   + muf::gg[idx]  )
	  + (QQN_1+QQN_2)/2.    *(hcoeff::Hqq[i]  *expc::qq[idx]   + muf::qq[idx]  )
	  + (QQPN_1+QQPN_2)/2.  *(hcoeff::Hqqp[i] *expc::qqp[idx]  + muf::qqp[idx] )
	  + (QQBPN_1+QQBPN_2)/2.*(hcoeff::Hqqbp[i]*expc::qqbp[idx] + muf::qqbp[idx]);
	    
      //contributions starting at NNNLL
      if (opts.order_hcoef >= 3)
	pos +=
	  + QQBN_nfz/2.         *hcoeff::Hqqb_nfz *expc::qqb[idx]
	  + (QBGN_1+QBGN_2)/2.  *(hcoeff::Hqbg[i] *expc::qbg[idx]  + muf::qbg[idx] )
	  + (QPGN_1+QPGN_2)/2.  *(hcoeff::Hqpg[i] *expc::qpg[idx]  + muf::qpg[idx] )
	  + (QBPGN_1+QBPGN_2)/2.*(hcoeff::Hqbpg[i]*expc::qbpg[idx] + muf::qbpg[idx]);

      //contributions starting at NNNNLL
      if (opts.order_hcoef >= 4)
	pos +=
	  +QBQN/2.               *(hcoeff::Hqbq[i]  *expc::qbq[idx]   + muf::qbq[idx]   )
	  +QPQBPN/2.             *(hcoeff::Hqpqbp[i]*expc::qpqbp[idx] + muf::qpqbp[idx] )
	  +QBPQPN/2.             *(hcoeff::Hqbpqp[i]*expc::qbpqp[idx] + muf::qbpqp[idx] )
	  +(QBQPN_1+QBQPN_2)/2.  *(hcoeff::Hqbqp[i] *expc::qbqp[idx]  + muf::qbqp[idx]  )
	  +(QBQBPN_1+QBQBPN_2)/2.*(hcoeff::Hqbqbp[i]*expc::qbqbp[idx] + muf::qbqbp[idx] )
	  +(QPQPN_1+QPQPN_2)/2.  *(hcoeff::Hqpqp[i] *expc::qpqp[idx]  + muf::qpqp[idx]  );
...
//      	   << endl;
      ...
      if(sector == "up" || sector== "UP" || sector== "Up")
	{
	  neg += + QQBNUP             *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
	  neg += + QQBNUP*             eq2*(conj(hcoeff::Hqqb_qed[i])*expc::qqbqed[idx]+muf::qqbqed[idx]);
	}
      else if (sector== "down" || sector == "DOWN" || sector == "Down") 
	{
	  neg += + QQBNDW             *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
	  neg += + QQBNDW*             eq2*(conj(hcoeff::Hqqb_qed[i])*expc::qqbqed[idx]+muf::qqbqed[idx]);
	}
      else
	{
	    cout << " wrong quark sector member in mellinint.C " << endl;
      ...
      // neg +=
      // 	//	QQBN               *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
      // 	+QQBNUP/2.               *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx])
      // 	+QQBNDW/2.               *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
     //contribution starting at NLL
      if (opts.order_hcoef >= 1)
	{
	  if(sector == "up" || sector== "UP" || sector== "Up")
	    {
	      neg +=
		+(QGNUP_1+QGNUP_2)  *(conj(hcoeff::Hqg[i])    *expc::qg[idx]   + muf::qg[idx]  );
	      if(opts.qedorder_hcoef >=1) neg +=+(QANUP_1+QANUP_2)*eq2 *(conj(hcoeff::Hqgamma[i]) *expc::qgamma[idx]   + muf::qgamma[idx]);
	    }
	  else if (sector== "down" || sector == "DOWN" || sector == "Down")  
	    {
	      neg +=+(QGNDW_1+QGNDW_2)  *(conj(hcoeff::Hqg[i])    *expc::qg[idx]   + muf::qg[idx]  );
	      if(opts.qedorder_hcoef >=1) neg += +(QANDW_1+QANDW_2)*eq2  *(conj(hcoeff::Hqgamma[i])*expc::qgamma[idx]   + muf::qgamma[idx]);
	    }
	  else
	    {
	      cout << " wrong quark sector member in mellinint.C " << endl;
      ...
      // if (opts.order_hcoef >= 1)
      // 	neg +=
      //        + (QGN_1+QGN_2)/2.    *(conj(hcoeff::Hqg[i]  ) *expc::qg[idx]   + muf::qg[idx]);

      //contributions starting at NNLL
      if (opts.order_hcoef >= 2)
	neg +=
	  + GGN/2.              *(conj(hcoeff::Hgg[i]  )*expc::gg[idx]    + muf::gg[idx]  )
	  + (QQN_1+QQN_2)/2.    *(conj(hcoeff::Hqq[i]  )*expc::qq[idx]    + muf::qq[idx]  )
	  + (QQPN_1+QQPN_2)/2.  *(conj(hcoeff::Hqqp[i] )*expc::qqp[idx]   + muf::qqp[idx] )
	  + (QQBPN_1+QQBPN_2)/2.*(conj(hcoeff::Hqqbp[i])*expc::qqbp[idx]  + muf::qqbp[idx]);
	    
      //contributions starting at NNNLL
      if (opts.order_hcoef >= 3)
	neg +=
	  + QQBN_nfz/2.         * conj(hcoeff::Hqqb_nfz)*expc::qqb[idx]
	  + (QBGN_1+QBGN_2)/2.  *(conj(hcoeff::Hqbg[i] )*expc::qbg[idx]   + muf::qbg[idx] )
	  + (QPGN_1+QPGN_2)/2.  *(conj(hcoeff::Hqpg[i] )*expc::qpg[idx]   + muf::qpg[idx] )
	  + (QBPGN_1+QBPGN_2)/2.*(conj(hcoeff::Hqbpg[i])*expc::qbpg[idx]  + muf::qbpg[idx]);

      //contributions starting at NNNNLL
      if (opts.order_hcoef >= 4)
	neg +=
	  +QBQN/2.               *(conj(hcoeff::Hqbq[i]  )*expc::qbq[idx]  + muf::qbq[idx]   )
	  +QPQBPN/2.             *(conj(hcoeff::Hqpqbp[i])*expc::qpqbp[idx]+ muf::qpqbp[idx] )
	  +QBPQPN/2.             *(conj(hcoeff::Hqbpqp[i])*expc::qbpqp[idx]+ muf::qbpqp[idx] )
	  +(QBQPN_1+QBQPN_2)/2.  *(conj(hcoeff::Hqbqp[i] )*expc::qbqp[idx] + muf::qbqp[idx]  )
	  +(QBQBPN_1+QBQBPN_2)/2.*(conj(hcoeff::Hqbqbp[i])*expc::qbqbp[idx]+ muf::qbqbp[idx] )
	  +(QPQPN_1+QPQPN_2)/2.  *(conj(hcoeff::Hqpqp[i] )*expc::qpqp[idx] + muf::qpqp[idx]  );
...
// ANDREA: Implementation of calc1d in the case of W boson production, no up-down separation.

complex <double> mellinint::calc1d(double eq2, double eqbarp2)
{
  complex <double> fun = 0.;
  int idx;
  for (int i = 0; i < mdim; i++)
    {
      //Positive branch
      // Attention with 1/2 factor! FOr W boson production we do not have up-down separation. Point to clarify for Z boson production: 1/2 starting from the NNLL.  
      complex <double> pos = 0.;
      idx = anomalous::index(i,mesq::positive);
      pdfevol::retrieve1d_pos(i);
      pdf_mesq_expy(i,i,mesq::positive);
      ...
      //contribution starting at LL      
      pos += + QQBN                  *(hcoeff::Hqqb[i]*expc::qqb[idx]+muf::qqb[idx]);
      pos += + QQBN* (eq2+eqbarp2)/2.*(hcoeff::Hqqb_qed[i]*expc::qqbqed[idx]+muf::qqbqed[idx]);     
      ...
      if (opts.order_hcoef >= 1)
	{
	  pos +=
	    +(QGN_1+QGN_2)*(hcoeff::Hqg[i]    *expc::qg[idx]   + muf::qg[idx]  );
	  if (opts.qedorder_hcoef >=1) pos +=+(QAN_1+QAN_2)*(eq2+eqbarp2)/2.*(hcoeff::Hqgamma[i]    *expc::qgamma[idx]   + muf::qgamma[idx]  );
	}    
...
//      	   << endl;
      ...
      //contribution starting at LL	
      neg += + QQBN             *(conj(hcoeff::Hqqb[i])*expc::qqb[idx]+muf::qqb[idx]);
      neg += + QQBN*       (eq2+eqbarp2)/2.*(conj(hcoeff::Hqqb_qed[i])*expc::qqbqed[idx]+muf::qqbqed[idx]);
      ...
      if (opts.order_hcoef >= 1)
	{
	  neg +=
	    +(QGN_1+QGN_2)  *(conj(hcoeff::Hqg[i])    *expc::qg[idx]   + muf::qg[idx]  );
	  if (opts.qedorder_hcoef >=1) neg +=+(QAN_1+QAN_2)*(eq2+eqbarp2)/2.*(conj(hcoeff::Hqgamma[i]) *expc::qgamma[idx]   + muf::qgamma[idx]);
	}

      //contributions starting at NNLL: remove the 1/2 factor because we do not have the separation up-down
      if (opts.order_hcoef >= 2)
	neg +=
	  + GGN             *(conj(hcoeff::Hgg[i]  )*expc::gg[idx]    + muf::gg[idx]  )
	  + (QQN_1+QQN_2)    *(conj(hcoeff::Hqq[i]  )*expc::qq[idx]    + muf::qq[idx]  )
...
complex <double> mellinint::calc2d(double eq2, std::string sector)
{ 
  complex <double> fun = 0.;
  bbfun = 0.;
  for (int i1 = 0; i1 < mdim; i1++)
      ...
      for (int i2 = 0; i2 < mdim; i2++)
	{
	  ...
	  if(sector == "up" || sector== "UP" || sector== "Up")
	    {
	      //contribution starting at LL 
	      fun += QQBNUP*(hcoeff::Hqqb[ii]*expc::qqb[ii]+muf::qqb[ii]);
	      fun += QQBNUP*eq2*(hcoeff::Hqqb_qed[ii]*expc::qqbqed[ii]+muf::qqbqed[ii]);
	    }
	  else if (sector== "down" || sector == "DOWN" || sector == "Down") 
	    {
	      //contribution starting at LL 
	      fun += QQBNDW*(hcoeff::Hqqb[ii]*expc::qqb[ii]+muf::qqb[ii]);
	      fun += QQBNDW*eq2*(hcoeff::Hqqb_qed[ii]*expc::qqbqed[ii]+muf::qqbqed[ii]);
	    }
	  else
	    {
	      cout << " wrong quark sector member in mellinint.C " << endl;
	  ...
	  if (opts.order_hcoef >= 1)
	    fun +=
	      + QGNUP_1  *(hcoeff::Hqg_1[ii]*expc::qg_1[ii]    +muf::qg_1[ii]  ) + QGNUP_2  *(hcoeff::Hqg_2[ii]  *expc::qg_2[ii]  +muf::qg_2[ii]);
	  if(opts.qedorder_hcoef >= 1) 
	    fun +=
	      + QANUP_1*eq2  *(hcoeff::Hqgamma_1[ii]*expc::qgamma_1[ii]    +muf::qgamma_1[ii]  ) + QANUP_2*eq2 *(hcoeff::Hqgamma_2[ii]  *expc::qgamma_2[ii]  +muf::qgamma_2[ii]);
	    }
	  else if (sector== "down" || sector == "DOWN" || sector == "Down") 
	    {
	  //contribution starting at NLL
	  if (opts.order_hcoef >= 1)
	    fun +=
	      + QGNDW_1  *(hcoeff::Hqg_1[ii]*expc::qg_1[ii]    +muf::qg_1[ii]  ) + QGNDW_2  *(hcoeff::Hqg_2[ii]  *expc::qg_2[ii]  +muf::qg_2[ii]);
	  if(opts.qedorder_hcoef >= 1) 
	    fun +=
	      + QANDW_1*eq2  *(hcoeff::Hqgamma_1[ii]*expc::qgamma_1[ii]    +muf::qgamma_1[ii]  ) + QANDW_2*eq2  *(hcoeff::Hqgamma_2[ii]  *expc::qgamma_2[ii]  +muf::qgamma_2[ii]);
	    }
	  else
	    {
	      cout << " wrong quark sector member in mellinint.C " << endl;
...
// Andrea, calc2d for W boson production, trial 
complex <double> mellinint::calc2d(double eq2, double eqbarp2)
{ 
  complex <double> fun = 0.;
  bbfun = 0.;
  for (int i1 = 0; i1 < mdim; i1++)
      ...
      for (int i2 = 0; i2 < mdim; i2++)
	{
	  ...
	  int ii = hcoeff::index(i1,i2,mesq::positive);
	      ...
	      //contribution starting at LL 
	      fun += QQBN*(hcoeff::Hqqb[ii]*expc::qqb[ii]+muf::qqb[ii]);
	      fun += QQBN*(eq2+eqbarp2)/2.*(hcoeff::Hqqb_qed[ii]*expc::qqbqed[ii]+muf::qqbqed[ii]);
	  ...
	  if(opts.qedorder_hcoef >= 1) 
	    fun +=
	      + QAN_1*(eq2+eqbarp2)/2.  *(hcoeff::Hqgamma_1[ii]*expc::qgamma_1[ii]    +muf::qgamma_1[ii]  ) + QAN_2*(eq2+eqbarp2)/2.*(hcoeff::Hqgamma_2[ii]  *expc::qgamma_2[ii]  +muf::qgamma_2[ii]);
	   

	  //contributions starting at NNLL
	  // Andrea: the 1/2 factorn is not present in case of W boson production 
	  if (opts.order_hcoef >= 2)
	    fun +=(
	      + GGN    *(hcoeff::Hgg[ii]    *expc::gg[ii]    +muf::gg[ii]    )
	      + QQN_1  *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  ) + QQN_2  *(hcoeff::Hqq_2[ii]  *expc::qq_2[ii]  +muf::qq_2[ii]  )
	  ...
	  //contributions starting at NNLL: remove /2 factor, as before
	  if (opts.order_hcoef >= 2)
	    fun -=(
	      + GGN    *(hcoeff::Hgg[ii]    *expc::gg[ii]    +muf::gg[ii]    )
	      + QQN_1  *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  ) + QQN_2  *(hcoeff::Hqq_2[ii]  *expc::qq_2[ii]  +muf::qq_2[ii]  )
...
complex <double> mellinint::calc2d_muf()
{
	...
	//   cout << "MELLININT calc2d_muf"<< endl; // CJU RES
	...
	if (opts.nproc == 4) {
		...
		for (int i1 = 0; i1 < mdim; i1++) {
			...
			for (int i2 = 0; i2 < mdim; i2++) {
				...
				// cout << "expc::gq_1 = " <<expc::gq_1[ii] << endl;
				// cout << "expc::gq_2 = " <<expc::gq_2[ii] << endl;
				/*
				 *	cout << "gg hcoeff = " <<hcoeff::Hgg[ii] << endl;
				 *	cout << "muf = " <<muf::gg[ii] << endl;
				 *	cout << "expc = " <<expc::gg[ii] << endl;
				 *
				 *	cout << "gq hcoeff = " <<hcoeff::Hgq_1[ii] << endl;
				 *	cout << "muf = " <<muf::gq_1[ii] << endl;
				 *	cout << "expc = " <<expc::gq_1[ii] << endl;
				 *	cout << "gq hcoeff = " <<hcoeff::Hgq_2[ii] << endl;
				 *	cout << "muf = " <<muf::gq_2[ii] << endl;
				 *	cout << "expc = " <<expc::gq_2[ii] << endl;
				 *
				 *	cout << "qq hcoeff = " <<hcoeff::Hqq_1[ii] << endl;
				 *	cout << "muf = " <<muf::qq_1[ii] << endl;
				 *	cout << "expc = " <<expc::qq_1[ii] << endl;
				 *	cout << "qq hcoeff = " <<hcoeff::Hqq_2[ii] << endl;
				 *	cout << "muf = " <<muf::qq_2[ii] << endl;
				 *	cout << "expc = " <<expc::qq_2[ii] << endl;
				 */

				//contribution starting at LL
				fun += GGN * (hcoeff::Hgg[ii]+ muf::gg[ii])*expc::gg[ii];

				//contribution starting at NLL
				if (opts.order_hcoef >= 1)
					fun +=
					+ GQN_1  *(hcoeff::Hgq_1[ii]*expc::gq_1[ii]    +muf::gq_1[ii]  *expc::gq_1[ii]) + GQN_2  *(hcoeff::Hgq_2[ii]*expc::gq_2[ii]  +muf::gq_2[ii]*expc::gq_1[ii]);

				// //contributions starting at NNLL
				if (opts.order_hcoef >= 2)
					fun +=
					+ QQN *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  *expc::qq_1[ii]);
				// + QQN_1  *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  *expc::qq_1[ii]);

				// CJU - N3LL
				if (opts.order_hcoef >= 3)
					// cout << "N3LL Hard coefficients not implemented" << endl;
					fun += 0.;
				// + QQN *(hcoeff::Hqq_1[ii]  *expc::qq_1[ii]  +muf::qq_1[ii]  *expc::qq_1[ii]);
				...
				if (opts.order_hcoef >= 2)
					fun -=
					+ QQN *(hcoeff::Hqq_2[ii]  *expc::qq_2[ii]  +muf::qq_2[ii]  *expc::qq_2[ii]);
				// + QQN_2  *(hcoeff::Hqq_2[ii]  *expc::qq_2[ii]  +muf::qq_2[ii]  *expc::qq_2[ii]);
