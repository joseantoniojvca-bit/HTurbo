...
double precision function ldli2(x)
  implicit none
...
double complex function cldli2(z)
  implicit none
  double complex :: z, rest, u, u2, u4, sum, cdlog1p
  double precision :: rz, iz, nz, sgn, ldli2
  double precision, parameter :: PI = 3.14159265358979324D0
  ...
  ! special cases
  if (iz .eq. 0) then
     if (rz .le. 1) cldli2 = dcmplx(ldli2(rz), iz)
     if (rz .gt. 1) cldli2 = dcmplx(ldli2(rz), -PI*log(rz))
     return
  endif
