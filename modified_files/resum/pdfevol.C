...
complex <double> *pdfevol::fotx1;
complex <double> *pdfevol::fotx2;

complex <double> pdfevol::fotn1;
complex <double> pdfevol::fotn2;
...
complex <double> pdfevol::logasl_b;
complex <double> pdfevol::asl_b;
complex <double> pdfevol::logasl_c;
complex <double> pdfevol::asl_c;

complex <double> pdfevol::logaqedl;
complex <double> pdfevol::aqedl;
...
void pdfevol::init()
{
  ...
  pmom::init();       //--> Needed for the gamma1,2,3,4 (moments of the splitting functions)
  //evolnum::init();
#ifdef USEHELLX
  hell::init();       //Read in x-space table
#endif
  evoltrunc::init();  //Init beta function
...
void pdfevol::release()
{
  ...
  //  case 4: hell::release();    break;
  //  }
}

void pdfevol::alphaqedl(complex <double> b)
{
  //.....Here computes NLL expression for alphaQED
  //     here nq2=b0^2/b^2 and the result is now  alpha(nq2)/alpha(muf2)  
  ...
  //blog is a log of Q/mub, with Q either mures or mufac, but the freezing is always as mures
  complex <double> mubtilde = btilde::mubtl(bstar,scales::res);
  complex <double> blog = 2.*log(Q/mubtilde);

  //Here should take alphaqed at the renormalization scale?
  complex <double> xlambda = alphaqed::beta0QED*aaqed*blog;

  // HERE now a dependence (without constant term)!

  // 20171211: Comentamos la contribucion proporcional a iord pues solo
  // aporta cuando vemos el NNLL (llegamos hasta NLL)
  logaqedl = log(1.-xlambda);
      
  aqedl = exp(-logaqedl);
  //cout << "alphaqed " << blog << "  " << xlambda << "  " << logaslqed << endl;
}
...
void pdfevol::alphasl(complex <double> b)
{
  ...
  //Force alphas to be in MSbar scheme
  double as = resint::aass; // = pdf::alphas(muren)/4./M_PI;

  //should change to:
  //as = pdf::alphas(scales::ren, false)/M_PI;
...
/*
  ...
  if (opts.evolmode == 2 || opts.evolmode == 3 || opts.commute)
    {
      Q = scales::fac;
      LQR = resint::LR - resint::LF; //not sure about this
    }
*/
  /*
  ...
  */
  ...
  //artificially remove muf scale variations
  //Q = scales::fac;
  //LQR = 0.;//resint::LR - resint::LF;
  //as = pdf::alphas(Q, false)/M_PI;
  ...
  /*
  //Set asyntotic scale to Q for commuting evolution
  if (opts.evolmode == 2 || opts.evolmode == 3 || opts.commute)
    {
      complex <double> mubtilde;
      if (!opts.modlog)
	mubtilde = resconst::b0/bstar;
      else if (opts.p == 1)
	mubtilde = resconst::b0/bstar * scales::res / sqrt((pow(resconst::b0/bstar,2) + pow(scales::res,2)));
      else
	mubtilde = resconst::b0/bstar * scales::res / pow((pow(resconst::b0/bstar,2*opts.p) + pow(scales::res,2*opts.p)),0.5/opts.p);

      blog = log(pow(Q/mubtilde,2));
    }
  */
  ...
  complex <double> xlambda = beta0_msbar*as*blog;
  complex <double> log1xlambda = log(1.-xlambda);
  //cout << " b " << b << " bstar " << bstar << " as " << as << " beta0_msbar " << beta0_msbar << " blog " << blog << " xlambda " << xlambda << endl;
  //  aexp = -Log[1 - lam]
  //    - aS/Pi*(beta1_msbar*Log[1 - lam] + beta0_msbar^2*lam*Log[Q2/muR2])/(beta0_msbar*(1 - lam))
  //    + aS^2/Pi^2*(-2*beta1_msbar^2*lam + 2*beta0_msbar*beta2_msbar*lam - 2*beta1_msbar^2*Log[1 - lam] + beta1_msbar^2*Log[1 - lam]^2 - 2*beta0_msbar^2*beta1_msbar*lam*Log[Q2/muR2] + 2*beta0_msbar^2*beta1_msbar*Log[1 - lam]*Log[Q2/muR2])/(2*beta0_msbar^2*(-1 + lam)^2)
  ...
  //  logasl += as*(LFQ)
  //    *beta0_msbar*xlambda/(1.-xlambda);
  //
  //if (opts.order_evol >= 3)
  //  logasl += as2*(+LFQ*beta1_msbar                   *(xlambda-log1xlambda)/pow(1.-xlambda,2)
  //		   +LFQ*beta1_msbar                   *xlambda/(1.-xlambda)                      //missing piece
  //		   +0.5*pow(LFQ,2)*pow(beta0_msbar,2) *xlambda*(xlambda-2.)/pow(1.-xlambda,2));  //missing piece
...
/*
  //HF evolution
  complex <double> bstar_b, bstar_c;
  if (opts.hfmode == 2)
    {
      bstar_b = bstar::calc(b,b0/(pdf::mb*opts.kmub));
      bstar_c = bstar::calc(b,b0/(pdf::mc*opts.kmuc));
    }
  else
    {
      bstar_b = bstar::bexp(b,b0/(pdf::mb*opts.kmub));
      bstar_c = bstar::bexp(b,b0/(pdf::mc*opts.kmuc));
    }
  
  //blog is a log of Q/mub, with Q either mures or mufac, but the freezing is always as mures
  complex <double> mubtilde_b = btilde::mubtl(bstar_b,scales::res);
  complex <double> mubtilde_c = btilde::mubtl(bstar_c,scales::res);
  complex <double> blog_b = 2.*log(Q/mubtilde_b);
  complex <double> blog_c = 2.*log(Q/mubtilde_c);

  xlambda = beta0_msbar*as*blog_b;
  log1xlambda = log(1.-xlambda);
  logasl_b = 0.;
  if (opts.order_evol >= 1)
    logasl_b += log1xlambda;
  if (opts.order_evol >= 2)
    logasl_b += as* beta1_msbar/beta0_msbar*log1xlambda/(1.-xlambda);
  if (opts.order_evol >= 3)
    logasl_b += as2* ((pow(beta1_msbar/beta0_msbar,2)-beta2_msbar/beta0_msbar) *xlambda/pow(1.-xlambda,2)
		      + pow(beta1_msbar/beta0_msbar,2)             *log1xlambda/pow(1.-xlambda,2)
		      - pow(beta1_msbar/beta0_msbar,2)             *pow(log1xlambda,2)/(2.*pow(1.-xlambda,2)));
  if (opts.order_evol >= 4)
    logasl_b += as3*
      (3.*xlambda*(pow(beta0_msbar,2)*beta3_msbar*(-2. + xlambda)
		   - 2.*beta0_msbar*beta1_msbar*beta2_msbar*(-1. + xlambda)
		   + pow(beta1_msbar,3)*xlambda)
       + 6.*(-(pow(beta1_msbar,3)*xlambda)
	     + beta0_msbar*beta1_msbar*beta2_msbar*(1. + xlambda))*log1xlambda
       - 9.*pow(beta1_msbar,3)*pow(log1xlambda,2)
       + 2.*pow(beta1_msbar,3)*pow(log1xlambda,3)
       )/(6.*pow(beta0_msbar,3)*pow(1. - xlambda,3));
  ...
  if (opts.order_evol >= 2)
    logasl_b += as*(LQR)
      *beta0_msbar*xlambda/(1.-xlambda);
  if (opts.order_evol >= 3)
    logasl_b += as2*(+LQR*beta1_msbar                   *(xlambda-log1xlambda)/pow(1.-xlambda,2)
		   +LQR*beta1_msbar                   *xlambda/(1.-xlambda)                      //missing piece
		   +0.5*pow(LQR,2)*pow(beta0_msbar,2) *xlambda*(xlambda-2.)/pow(1.-xlambda,2));  //missing piece
  if (opts.order_evol >= 4)
    logasl_b += as3*
      (LQR*(6.*pow(beta1_msbar,2)*pow(log1xlambda,2)
	    + 6.*beta1_msbar*log1xlambda*(-3.*beta1_msbar + pow(beta0_msbar,2)*LQR)
	    + xlambda*(-6.*pow(beta1_msbar,2) + 6.*beta0_msbar*beta2_msbar*(4. - 3.*xlambda + pow(xlambda,2))
		       - 9.*pow(beta0_msbar,2)*beta1_msbar*(3. - 3.*xlambda + pow(xlambda,2))*LQR
		       + 2.*pow(beta0_msbar,4)*(3. - 3.*xlambda + pow(xlambda,2))*pow(LQR,2))))/(6.*beta0_msbar*pow(1. - xlambda,3));

  xlambda = beta0_msbar*as*blog_c;
  log1xlambda = log(1.-xlambda);
  logasl_c = 0.;
  if (opts.order_evol >= 1)
    logasl_c += log1xlambda;
  if (opts.order_evol >= 2)
    logasl_c += as* beta1_msbar/beta0_msbar*log1xlambda/(1.-xlambda);
  if (opts.order_evol >= 3)
    logasl_c += as2* ((pow(beta1_msbar/beta0_msbar,2)-beta2_msbar/beta0_msbar) *xlambda/pow(1.-xlambda,2)
		      + pow(beta1_msbar/beta0_msbar,2)             *log1xlambda/pow(1.-xlambda,2)
		      - pow(beta1_msbar/beta0_msbar,2)             *pow(log1xlambda,2)/(2.*pow(1.-xlambda,2)));
  if (opts.order_evol >= 4)
    logasl_c += as3*
  ...
  if (opts.order_evol >= 2)
    logasl_c += as*(LQR)
      *beta0_msbar*xlambda/(1.-xlambda);
  if (opts.order_evol >= 3)
    logasl_c += as2*(+LQR*beta1_msbar                   *(xlambda-log1xlambda)/pow(1.-xlambda,2)
		   +LQR*beta1_msbar                   *xlambda/(1.-xlambda)                      //missing piece
		   +0.5*pow(LQR,2)*pow(beta0_msbar,2) *xlambda*(xlambda-2.)/pow(1.-xlambda,2));  //missing piece
  if (opts.order_evol >= 4)
    logasl_c += as3*
...
void pdfevol::evolution()
{
  switch (opts.evolmode)
    ...
    //PDF evolution in Mellin space from Q to b0/b
    case 5: evoltrunc::evolve();    break;
    }
}
...
void pdfevol::allocate()
{
  ...
  if (opts.evolmode == 1 || opts.evolmode == 3 || (opts.evolmode == 4 && opts.commute))
    if (opts.melup == 2)
      pegasus::allocate();

#ifdef USEHELLX
  if (opts.evolmode == 4) // || opts.evolmode == 5)
    {
      //if (opts.melup == 2)
      hell::allocate();
      hell::allocateusg();
  ...
  if (opts.evolmode == 4 || opts.evolmode == 5 || opts.evolmode == 2 || opts.evolmode == 3 || opts.commute)
    if (opts.melup == 2)
      evoltrunc::allocate();
}
...
void pdfevol::update()
{
  ...
  //if (opts.evolmode == 1 || (opts.evolmode == 4 && opts.commute))
  if (opts.evolmode == 1)
    pegasus::update();
...
void pdfevol::storemoments_b(int i, int sign, complex <double> fx[11])
{
  int nf = 2*MAXNF+1;
  if (sign == mesq::positive)
  {
...
void pdfevol::storemoments_c(int i, int sign, complex <double> fx[11])
{
  int nf = 2*MAXNF+1;
  if (sign == mesq::positive)
  {
...
void pdfevol::retrieve1d_neg()
{
  ...
  //  fotn1 = fotx1[mellinint::mdim+i];
  //  fotn2 = fotx2[mellinint::mdim+i];
}

void pdfevol::retrieve1d_neg(int i)
{
  retrieve1d_pos(i);
  retrieve1d_neg();
}
...
void pdfevol::retrievemuf(int i, int j, int sign)
{
  // i is the index of the complex mellin moment in the z-space for the gaussian quadrature used for the mellin inversion
  ...
  fx_1[ 4+MAXNF] = evolnative::CVP[i] + evolnative::CHP[i];
  fx_1[ 5+MAXNF] = evolnative::BVP[i] + evolnative::BOP[i];
  ft_1           = evolnative::FOP[i];

  complex <double> fx_2[11], ft_2;
  fx_2[-5+MAXNF] = evolnative::BOP[j+mellinint::mdim];
  fx_2[-4+MAXNF] = evolnative::CHP[j+mellinint::mdim];
  fx_2[-3+MAXNF] = evolnative::SSP[j+mellinint::mdim];
  fx_2[-2+MAXNF] = evolnative::DSP[j+mellinint::mdim];
  fx_2[-1+MAXNF] = evolnative::USP[j+mellinint::mdim];
  fx_2[ 0+MAXNF] = evolnative::GLP[j+mellinint::mdim];
  fx_2[ 1+MAXNF] = evolnative::UVP[j+mellinint::mdim] + evolnative::USP[j+mellinint::mdim];
  fx_2[ 2+MAXNF] = evolnative::DVP[j+mellinint::mdim] + evolnative::DSP[j+mellinint::mdim];
  fx_2[ 3+MAXNF] = evolnative::SVP[j+mellinint::mdim] + evolnative::SSP[j+mellinint::mdim];
  fx_2[ 4+MAXNF] = evolnative::CVP[j+mellinint::mdim] + evolnative::CHP[j+mellinint::mdim];
  fx_2[ 5+MAXNF] = evolnative::BVP[j+mellinint::mdim] + evolnative::BOP[j+mellinint::mdim];
  ft_2           = evolnative::FOP[j+mellinint::mdim];
