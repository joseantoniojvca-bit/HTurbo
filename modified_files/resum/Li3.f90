...
! Li_3(x) for x in [-1,0]
double precision function ldli3_neg(x)
  implicit none
...
end function ldli3_neg


! Li_3(x) for x in [0,1/2]
double precision function ldli3_pos(x)
  implicit none
  double precision :: x, x2, x4, p, q
  double precision, parameter :: cp(6) = (/                &
...
!*********************************************************************
double precision function ldli3(x)
  implicit none
  double precision :: x, l, ldli3_neg, ldli3_pos
  ...
  if (x .lt. -1) then
     l = log(-x)
     ldli3 = ldli3_neg(1/x) - l*(zeta2 + 1.0D0/6*l**2)
  elseif (x .eq. -1) then
     ldli3 = -0.75D0*zeta3
  elseif (x .lt. 0) then
     ldli3 = ldli3_neg(x)
  ...
  elseif (x .eq. 0.5D0) then
     ldli3 = 0.53721319360804020D0
  elseif (x .lt. 1) then
     l = log(x)
     ldli3 = -ldli3_neg(1 - 1/x) - ldli3_pos(1 - x) + &
          zeta3 + l*(zeta2 + l*(-0.5D0*log(1 - x) + 1.0D0/6*l))
  elseif (x .eq. 1) then
     ldli3 = zeta3
  elseif (x .lt. 2) then
     l = log(x)
     ldli3 = -ldli3_neg(1 - x) - ldli3_pos(1 - 1/x) + &
          zeta3 + l*(zeta2 + l*(-0.5D0*log(x - 1) + 1.0D0/6*l))
  else ! x >= 2.0D0
     l = log(x)
     ldli3 = ldli3_pos(1/x) + l*(2*zeta2 - 1.0D0/6*l**2)
  endif
...
!> @author Alexander Voigt
!*********************************************************************
double complex function cldli3(z)
  implicit none
  double complex :: z, u, u2, u4, u8, c0, c1, lmz, rest, pos_cdlog
  double precision :: rz, iz, nz, pz, lnz, arg, ldli3
  ...
  iz = aimag(z)
   ...
   if (iz .eq. 0) then
      if (rz .le. 1) then
         cldli3 = dcmplx(ldli3(rz), iz)
         return
      else
         lnz = log(rz)
         cldli3 = dcmplx(ldli3(rz), -0.5D0*PI*lnz**2)
         return
      endif
   endif
