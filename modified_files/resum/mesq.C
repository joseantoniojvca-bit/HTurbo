...
// New definitions LEAN
#define ICPLX std::complex<double>(0,1)

// contants (quark charges)
...
void mesq::init()
{
  ...
  if (opts.ewscheme == 4)
    {
      gW = sqrt(4.*M_PI*coupling::aemmz/coupling::xw);
      gZ = gW/(2.*sqrt(1.-coupling::xw));
    }
...
void mesq::setmesq(T one, T costh1, T costh2)
{
  ...
  if (opts.nproc == 3 && opts.useGamma) // Z/gamma*
    ...
    // CJU RES Higgs production
    if (opts.nproc == 4) {

      double sigma0;

      if(opts.calc_type == "Sig" || opts.calc_type == "sig" || opts.calc_type == "Int" ||
        opts.calc_type == "int" || opts.calc_type == "SigInt" || opts.calc_type == "sigInt" ||
        opts.calc_type == "sigint")
      {

        // New implementation
        //  cout << "mtop : " << opts.mtop << endl;
        // cout << "mbottom : " << opts.mbottom << endl;
        // cout << "mcharm : " << opts.mcharm << endl;
        // cout << "mtau : " << opts.mtau << endl;
        // cout << "calc_type : " << opts.calc_type << endl;
        // cout << "removebr : " << opts.removebr << endl;
        // cout << "decay_type : " << opts.decay_type << endl;

        T t = -(1. + 1.*costh1)/2.*phasespace::m*phasespace::m;
        T u = -phasespace::m*phasespace::m -t;
        T s = - t - u;
        ...
        qcdcouple_.as_ = pdf::rgktalphas(scales::ren);

        complex<double> sigma_LO = amp_MggHyy( real(s), real(t), real(u));
         mesqij[0] = sigma_LO * gevfb / pow(2.0,7.0)/s/M_PI * pow(qcdcouple_.as_, 2);
        //	mesqij[0] = sigma_LO * gevfb * pow(0.112, 2) / pow(2.0,7.0)/s/M_PI;
         //   cout << "  sigma_LO : " <<  sigma_LO << endl;
        //	cout <<" qcdcouple_.as_ " << qcdcouple_.as_ << endl;
         //   cout << "  mesq : " <<  mesqij[0] << endl;
      ...
      else
      {
        ...
        // OLD implementation
        // ------------------------------------------------------------------------------------
        // Normalization sigma0
        double norm_higgs = opts.rmass / (opts.rwidth) * (4./3./M_PI);
        double factor_higgs = opts.Gf / (288 * M_PI * sqrt(2));
        sigma0 = norm_higgs * factor_higgs * gevfb * pow(qcdcouple_.as_, 2); // as from scales.C
        // double sigma0 = norm_higgs * factor_higgs * gevfb * pow(resint::aass * M_PI, 2); // as from resint.C

        //      cout << "opts.rmass : " << opts.rmass << "  " << "opts.rwidth : " << opts.rwidth << endl;
        // Higgs matrix amplitude
        mesqij[0] = sigma0 * one;
      }

    }

  if (opts.nproc == 3 && opts.useGamma) // only gamma*
    {
      mesqij[0]=fac*propG*aem2pi2*pow(Q[1],2) * (one + costh2);
      mesqij[6]=fac*propG*aem2pi2*pow(Q[3],2) * (one + costh2);
      mesqij[1]=fac*propG*aem2pi2*pow(Q[1],2) * (one + costh2);
      mesqij[7]=fac*propG*aem2pi2*pow(Q[3],2) * (one + costh2);
      mesqij[2]=fac*propG*aem2pi2*pow(Q[0],2) * (one + costh2);
      mesqij[4]=fac*propG*aem2pi2*pow(Q[2],2) * (one + costh2);
      mesqij[8]=fac*propG*aem2pi2*pow(Q[4],2) * (one + costh2);
      mesqij[3]=fac*propG*aem2pi2*pow(Q[0],2) * (one + costh2);
      mesqij[5]=fac*propG*aem2pi2*pow(Q[2],2) * (one + costh2);
      mesqij[9]=fac*propG*aem2pi2*pow(Q[4],2) * (one + costh2);
    }
...
std::complex<double> mesq::LO_MggH(double s12)
{
  //Este elemento de matriz calcula la amplitud de Higgs gg->H
  // SumaMartin controla el tipo de vertice y que contribuciones incluye
  double _VEV_ = sqrt(1./opts.Gf/sqrt(2.0));
  //  cout << " _VEV_ : " << _VEV_ << endl;
  double vmartin = _VEV_/sqrt(2.0);
  std::complex<double>   Aggh =-s12/(8.0*sqrt(2.0)*M_PI*vmartin)*mesq::SumaMartin(s12);
  return Aggh ;
}

std::complex<double> mesq::LO_MHyy(double s12)
{
  //Este elemento de matriz calcula la amplitud de decaimiento del Higgs H->yy
  // CorcheteMartin controla que elementos incluye el vertice y que tipo es
  double _VEV_ = sqrt(1./opts.Gf/sqrt(2.0));
  double vmartin = _VEV_/sqrt(2.0);
  //cout << "opts.aemmz = " << 1./opts.aemmz << endl;
  std::complex<double>   Ahyy =  -(opts.aemmz*s12)/(4.0*sqrt(2.0)*M_PI*vmartin)*mesq::CorcheteMartin(s12);
  return Ahyy ;
}

double mesq::LO_Mggyy(double s12, double t, double u)
{
  ...
  double Aggyy =  2.0*opts.aemmz*(-(((u-t)*log(u/t))/s12)
  -((t*t + u*u)*(M_PI*M_PI+ log(u/t)*log(u/t)))/(2.0*s12*s12))*
  11.0/9.0; //alpha_s to be included at final moment
  //  std::cout << "Aggyy  :" << Aggyy  << std::endl;
  return  Aggyy ;
}

std::complex<double> mesq::amp_MggHyy(double s, double t, double u)
{
  // Esta funcion calcula la amplitud modulo quadro para los casos de la señal  del higgs,
  // la interferencia con el background de dos fotones o la señal mas la interferencia
  // double s = real(sc);
  // double t = real(tc);
  // double u = real(uc);
  ...
  // u = -5909.13;
  // background part:
  double Aggyyp  = LO_Mggyy(s,t,u);
  // only the left part of the diagram
  std::complex<double> Aggh   = LO_MggH(s);
  std::complex<double> Aggyy = std::complex<double> (Aggyyp,0.0);
  // decayment
  std::complex<double> Ahyy   = LO_MHyy(s);
  // Interference of the full process (between the box diagram and the triangle diagram with a Higgs)
  std::complex<double> N_int  = Aggh*Ahyy*conj(Aggyy);
  double N_int_Re = real(N_int);
  double N_Int_Im = imag(N_int);
  double AtotR=0.0;
  ...
  string Contrib_Inter = opts.calc_type;
  double Ahyy_for_br;
  double gamgam_br = 1.;
  // total cross section opts.removebr==true
  if(opts.removebr == true || opts.decay_type == "ihixs")
  {
    Ahyy_for_br = real(LO_MHyy(mh*mh))*real(LO_MHyy(mh*mh))/16./M_PI/mh;
    gamgam_br = Ahyy_for_br/Gammah;
  }

  // cout << " gamgam_br : " << gamgam_br << endl;
  // cout << " mh : " << mh << endl;
  // cout << " Gammah : " <<  Gammah << endl;
  // cout << " coupling::aemmz : " << coupling::aemmz << endl;
  double _VEV_ = sqrt(1./opts.Gf/sqrt(2.0));
  double vmartin = _VEV_/sqrt(2.0);
  // Higgs propagator:
  double Ds = ((-mh*mh + s)*(-mh*mh + s) + mh*mh*Gammah*Gammah);
  if (Contrib_Inter == "Sig" || Contrib_Inter == "sig")
  {
    AtotR=abs((Ahyy*Aggh)*conj(Ahyy*Aggh) );
    //  std::complex<double> nostra = (mesq::CorcheteMartin(s))*(mesq::SumaMartin(s))*conj((mesq::CorcheteMartin(s))*(mesq::SumaMartin(s)))*((-2. + 4.)*(-2. + 4.)*(-1. + 3.*3.)*s*s*s*s)/(16.*(Gammah*mh*Gammah*mh + (mh*mh - s)*(mh*mh - s)));;

    //      cout << " ratio = " << 2.*real(nostra)*opts.aemmz*opts.aemmz/vmartin/vmartin/vmartin/vmartin/pow(8.*sqrt(2.)*M_PI,4.)/real(AtotR)*Ds << endl;

    //      cout << " Li2(s12) = " << Li2(s) << endl;

    //std::cout << AtotR << std::endl;
  }
  else if (Contrib_Inter == "Int" || Contrib_Inter == "int")
  {
    AtotR= - 2.0*(s-mh*mh)*N_int_Re - 2.0*mh*Gammah*N_Int_Im;
  }
  else if(Contrib_Inter == "SigInt" || Contrib_Inter == "Sigint" || Contrib_Inter == "sigInt" || Contrib_Inter == "sigint")
  {
  ...
  if(opts.removebr == true) return std::complex<double>(AtotR/Ds/gamgam_br,0.);
  // good only near the Higgs mass
  return AtotR/Ds;
}

double mesq::_amp_MggHyy(double s, double t, double u)
{

  // Esta funcion calcula la amplitud modulo quadro para los casos de la señal  del higgs,
  // la interferencia con el background de dos fotones o la se coupling::aemmz = 1./137.ñal mas la interferencia
  // double s = real(sc);
  // double t = real(tc);
  // double u = real(uc);
  ...
  double Ds = ((-mh*mh + s)*(-mh*mh + s) + mh*mh*Gammah*Gammah);
  // cout << " Ds from _amp_MggHyy : " << Ds << endl;
  // cout << " s = " << s << endl;
  // cout << " mh = " << mh << endl;
  // cout << " Gammah : " << Gammah << endl;
  ...
  if(opts.removebr == true) return AtotR/gamgam_br;
  return real(AtotR);
}

std::complex<double> mesq::CorcheteMartin(double ss)
{
  // Vertice correspondiente al decaimeinto del Higgs en dos fotones
  double mW = opts.wmass;
  double mt = opts.mtop;
  double mb = opts.mbottom;
  double mc = opts.mcharm;
  double mtau = opts.mtau;
  string decay_h =  opts.decay_type;
  if( decay_h == "notau_noc")
    return  mesq::F1(4.0*mW*mW/ss)+ 3.0*( (2.0/3.0)*(2.0/3.0)*mesq::F1on2(4.0*mt*mt/ss) + (-1.0/3.0)*(-1.0/3.0)*mesq::F1on2(4.0*mb*mb/ss));
  else if( decay_h == "only_top")
    return mesq::F1(4.0*mW*mW/ss)+3.0*((2.0/3.0)*(2.0/3.0)*mesq::F1on2(4.0*mt*mt/ss));
  else if( decay_h == "mcfm" || decay_h == "ihixs")
    return  mesq::F1(4.0*mW*mW/ss)+ 3.0*( (2.0/3.0)*(2.0/3.0)*mesq::F1on2(4.0*mt*mt/ss) + (-1.0/3.0)*(-1.0/3.0)*mesq::F1on2(4.0*mb*mb/ss));
  // full vertex
  return mesq::F1on2(4.0*mtau*mtau/ss) + F1(4.0*mW*mW/ss)+ 3.0*( (2.0/3.0)*(2.0/3.0)*mesq::F1on2(4.0*mc*mc/ss) + (2.0/3.0)*(2.0/3.0)*mesq::F1on2(4.0*mt*mt/ss) + (-1.0/3.0)*(-1.0/3.0)*mesq::F1on2(4.0*mb*mb/ss));
}

std::complex<double> mesq::SumaMartin(double ss)
{
  // Vertice correspondiente al decaimeinto del Higgs en dos fotones
  double mt = (opts.mtop);
  if(opts.prodmtinf) return complex<double>(-4./3.,0.);
  double mb = opts.mbottom;
  double mc = opts.mcharm;
  string decay_h =  opts.decay_type;
  if( decay_h == "only_top") return mesq::F1on2(4.0*mt*mt/ss);
  else if ( decay_h == "notau_noc") return mesq::F1on2(4.0*mt*mt/ss)+mesq::F1on2(4.0*mb*mb/ss);
  else if ( decay_h == "mcfm") return mesq::F1on2(4.0*mt*mt/ss);
  else if( decay_h == "ihixs") return complex<double>(4./3.,0.);
  // full vertex
  return (mesq::F1on2(4.0*mt*mt/ss)+mesq::F1on2(4.0*mb*mb/ss)+mesq::F1on2(4.0*mc*mc/ss));
}

std::complex<double> mesq::ff( double xx)
{
  if( xx >= 1.0 ) return asin( sqrt(1.0/xx) )*asin( sqrt(1.0/xx) );
  return -1.0/4.0*(  log((1.0+sqrt(1.0-xx))/(1.0-sqrt(1.0-xx))) - ICPLX* M_PI)*(  log((1.0+sqrt(1.0-xx))/(1.0-sqrt(1.0-xx))) - ICPLX* M_PI);
}

std::complex<double> mesq::F1( double xx)
{
  return 2.0 + 3.0*xx*(1.0+(2.0-xx)*ff(xx));
}

std::complex<double> mesq::F1on2( double xx)
{
  return -2.0*xx*(1.0+(1.0-xx)*ff(xx));
}

double mesq::LQ(double costh1)
{
  double mtop = opts.mtop;
  double t = -(1. - 1.*costh1)/2.*phasespace::m*phasespace::m;
  double u = -phasespace::m*phasespace::m -t;
  double s = - t - u;
  return log(s/mtop);
}
