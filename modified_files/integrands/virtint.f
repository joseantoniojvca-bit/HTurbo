      ...
      double precision qq2,dytdot
      double precision f(*)

      integer ih1,ih2,j,k,cs,nvec,is,ia,ib,ic
      double precision p(mcfm_mxpart,4),pjet(mcfm_mxpart,4),r(mxdim),W,sqrts,xmsq,
     . val,fx1(-nf:nf),fx2(-nf:nf),fx1z(-nf:nf),fx2z(-nf:nf)
      double precision ft1,ft2
...
C      external hists_fill_PDF
      ...
      double precision convolute_qed
...
c---impose mass cuts on final state --> Not needed because the phase space of p34 is generated between wsqmin and wsqmax
      call masscuts(s,*999)
...
c--- bother calculating the matrix elements for it, instead bail out
      if (mcfm_includedipole(0,p) .eqv. .false.) then
         return
      endif
...
c     intitialise xmsq to 0
         ...
         if (z .gt. xx(1)) then
            x1onz=xx(1)/z
            call pdf_fdist(1,x1onz,facscale,fx1z)
         endif
         if (z .gt. xx(2)) then
            x2onz=xx(2)/z
            call pdf_fdist(2,x2onz,facscale,fx2z)
         endif         
...
c     QED
      call fdistphot(1,xx(1),facscale, ft1)
      call fdistphot(2,xx(2),facscale, ft2)
      xmsq = xmsq + convolute_qed(fx1,fx2,ft1,ft2,msq)
